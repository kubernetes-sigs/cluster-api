...
func TestReconcileUpdateObservedGeneration(t *testing.T) {
	...
	r := &Reconciler{
		...
		SecretCachingClient: secretCachingClient,
		recorder:            record.NewFakeRecorder(32),
		managementCluster:   &pkg.Management{Client: env.Client, ClusterCache: nil},
	}
...
func TestReconcileClusterNoEndpoints(t *testing.T) {
	...
	r := &Reconciler{
		...
		managementCluster: &fakeManagementCluster{
			Management: &pkg.Management{Client: fakeClient},
	...
	tests := []struct {
		...
		},
		{
			...
			controlPlane: &pkg.ControlPlane{
				...
				Nodes: []*pkg.Node{
					pkg.TransformNode(&corev1.Node{ObjectMeta: metav1.ObjectMeta{
						Name:              "m2-node",
