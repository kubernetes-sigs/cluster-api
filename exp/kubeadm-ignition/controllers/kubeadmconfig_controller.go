...
import (
	"context"
	"fmt"
	kubeadmignition "sigs.k8s.io/cluster-api/exp/kubeadm-ignition/internal/ignition"
	"strconv"
	"strings"
	"time"
...
func (r *KubeadmIgnitionConfigReconciler) Reconcile(req ctrl.Request) (_ ctrl.Result, rerr error) {
	...
	// Attempt to Patch the KubeadmIgnitionConfig object and status after each reconciliation if no error occurs.
	defer func() {
		if rerr == nil {
...
func (r *KubeadmIgnitionConfigReconciler) handleClusterNotInitialized(ctx context.Context, scope *Scope) (_ ctrl.Result, reterr error) {
	...
	err = certificates.LookupOrGenerate(
		...
		*metav1.NewControllerRef(scope.Config, bootstrapv1.GroupVersion.WithKind("KubeadmIgnitionConfig")),
	)
	if err != nil {
	...
	cloudInitData, err := r.IgnitionFactory.GenerateUserData(&kubeadmignition.Node{
		Files: append(certificates.AsFiles(), bootstrapv1.File{
			Path:        kubeadmignitionv1beta1.KubeadmIgnitionConfigPath,
			Permissions: "0640",
			Content:     strings.Join([]string{initdata, clusterdata}, "\n---\n"),
		}),
		Services: []kubeadmignitionv1beta1.ServiceUnit{
			{
				Name:    "kubeinit.service",
				Content: fmt.Sprintf(kubeadmignitionv1beta1.InitUnitTemplate, verbosityFlag, kubeadmignitionv1beta1.KubeadmIgnitionConfigPath),
				Enabled: true,
				Dropins: kubeadmignitionv1beta1.GetCommandsDropins(scope.Config.Spec.PreKubeadmCommands, scope.Config.Spec.PostKubeadmCommands),
			},
		},
...
func (r *KubeadmIgnitionConfigReconciler) joinWorker(ctx context.Context, scope *Scope) (ctrl.Result, error) {
	...
	if scope.Config.Spec.JoinConfiguration.ControlPlane != nil {
		return ctrl.Result{}, errors.New("Machine is a Worker, but JoinConfiguration.ControlPlane is set in the KubeadmIgnitionConfig object")
	}
	...
	cloudJoinData, err := r.IgnitionFactory.GenerateUserData(&kubeadmignition.Node{
		Files: append(certificates.AsFiles(), bootstrapv1.File{
			Path:        kubeadmignitionv1beta1.KubeadmIgnitionConfigPath,
			Permissions: "0640",
			Content:     joinData,
		...
		Services: []kubeadmignitionv1beta1.ServiceUnit{
			{
				Name:    "kubeinit.service",
				Enabled: true,
				Content: fmt.Sprintf(kubeadmignitionv1beta1.JoinUnitTemplate, verbosityFlag, kubeadmignitionv1beta1.KubeadmIgnitionConfigPath),
				Dropins: kubeadmignitionv1beta1.GetCommandsDropins(scope.Config.Spec.PreKubeadmCommands, scope.Config.Spec.PostKubeadmCommands),
			},
		},
		Version: scope.Config.Spec.ClusterConfiguration.KubernetesVersion,
...
func (r *KubeadmIgnitionConfigReconciler) joinControlplane(ctx context.Context, scope *Scope) (ctrl.Result, error) {
	if !scope.ConfigOwner.IsControlPlaneMachine() {
...
func (r *KubeadmIgnitionConfigReconciler) ClusterToKubeadmIgnitionConfigs(o handler.MapObject) []ctrl.Request {
	...
	for _, m := range machineList.Items {
		if m.Spec.Bootstrap.ConfigRef != nil &&
			m.Spec.Bootstrap.ConfigRef.GroupVersionKind().GroupKind() == bootstrapv1.GroupVersion.WithKind("KubeadmIgnitionConfig").GroupKind() {
...
func (r *KubeadmIgnitionConfigReconciler) storeBootstrapData(ctx context.Context, scope *Scope, data []byte) error {
	...
	if err := r.Client.Create(ctx, secret); err != nil {
		return errors.Wrapf(err, "failed to create bootstrap data secret for KubeadmIgnitionConfig %s/%s", scope.Config.Namespace, scope.Config.Name)
	}
