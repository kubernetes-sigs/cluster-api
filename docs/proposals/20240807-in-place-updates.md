...
When `Stop` is set, the cluster may remain not-fully-updated until the user intervenes. Rather than adding a new condition, the blocked state is surfaced by reusing existing conditions:

For example, on a blocked Machine:
...
status:
  ...
  - type: UpToDate
    status: "False"
    reason: InPlaceUpdateBlocked
    message: "In-place update cannot proceed (extension cannot cover the full diff) and inPlaceFallback is Stop; rollout is paused pending intervention."
```

which aggregates on the owning `KubeadmControlPlane` / `MachineDeployment`:
...
status:
  ...
  - type: MachinesUpToDate
    status: "False"
    reason: NotUpToDate
    message: "Machine <name>: In-place update cannot proceed and inPlaceFallback is Stop; rollout is paused pending intervention."
```

The exact condition reason naming can be refined during implementation.
