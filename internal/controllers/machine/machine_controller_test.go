...
func TestMachineConditions(t *testing.T) {
	...
	testcases := []struct {
		...
		{
			name:           "ready condition summary consumes reason from the draining succeeded condition when pod eviction starts",
			infraReady:     true,
			bootstrapReady: true,
			beforeFunc: func(_, _ *unstructured.Unstructured, m *clusterv1.Machine) {
				callback := onPodDeletionOrEvictionStarted(context.Background(), m)
				callback(pod, true)
			},
			conditionsToAssert: []*clusterv1.Condition{
				conditions.FalseCondition(clusterv1.ReadyCondition, clusterv1.DrainingReason, clusterv1.ConditionSeverityInfo, "Eviction of pod test-pod started from node test"),
			},
		},
		{
			name:           "ready condition summary consumes reason from the draining succeeded condition when pod deletion starts",
			infraReady:     true,
			bootstrapReady: true,
			beforeFunc: func(_, _ *unstructured.Unstructured, m *clusterv1.Machine) {
				callback := onPodDeletionOrEvictionStarted(context.Background(), m)
			...
			conditionsToAssert: []*clusterv1.Condition{
				conditions.FalseCondition(clusterv1.ReadyCondition, clusterv1.DrainingReason, clusterv1.ConditionSeverityInfo, "Deletion of pod test-pod started from node test"),
			},
		},
		{
