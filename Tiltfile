...
def load_addon_tiltfiles():
    ...
    for repo in addon_repos:
        ...
        addon_settings_files.append(file)
...
def prepare_all():
    ...
    kustomize_substitutions = settings.get("kustomize_substitutions", {})

    addon_settings_file_arg = ""
    for f in addon_settings_files:
        addon_settings_file_arg = addon_settings_file_arg + " --addon-settings-file " + f

    cmd = "make -B tilt-prepare && ./hack/tools/bin/tilt-prepare {tools_arg}{kustomize_substitutions}{tilt_settings_file_arg}{addon_settings_file_arg}".format(
        tools_arg = tools_arg,
        tilt_settings_file_arg = tilt_settings_file_arg,
        kustomize_substitutions = kustomize_substitutions,
        addon_settings_file_arg = addon_settings_file_arg,
    )
