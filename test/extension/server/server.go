...
func (s *Server) Start(ctx context.Context) error {
	...
	for _, h := range s.handlers {
		...
		r.HandleFunc(handlerPath, wrappedHandler).Methods("POST")
	}

	s.server.WebhookMux.Handle("/", r)

	return s.server.StartStandalone(ctx, nil)
}

// discoveryHandler generates a discovery handler based on a list of handlers.
func discoveryHandler(handlers []ExtensionHandler) func(context.Context, *runtimehooksv1.DiscoveryRequest, *runtimehooksv1.DiscoveryResponse) {
