...
func Test_providerUpgrader_getUpgradeInfo(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "pass when current and next version contract are v1alpha4",
			fields: fields{
		...
		{
			name: "pass when current version is v1alpha3 contract, next v1alpha3 (upgrade plan should work, but apply should block)",
			fields: fields{
				reader: test.NewFakeReader().
					WithProvider("p1", clusterctlv1.InfrastructureProviderType, "https://somewhere.com"),
				repository: test.NewFakeRepository().
		...
		{
			name: "pass when current v1alpha4 is contract, next v1alpha5 (it should fall back to latest known metadata)",
			fields: fields{
				reader: test.NewFakeReader().
					WithProvider("p1", clusterctlv1.InfrastructureProviderType, "https://somewhere.com"),
				repository: test.NewFakeRepository().
			...
			want: &upgradeInfo{
				metadata: &clusterctlv1.Metadata{
					...
					ReleaseSeries: []clusterctlv1.ReleaseSeries{
						{Major: 1, Minor: 0, Contract: "v1alpha4"}, // Fallback to latest known metadata
					},
				},
				currentVersion:  version.MustParseSemantic("v1.0.1"),
				currentContract: "v1alpha4",
				nextVersions: []version.Version{
					// v1.0.1 (the current version) and older are ignored
					*version.MustParseSemantic("v1.0.2"),
					*version.MustParseSemantic("v1.1.0"), // this version is kept, but it will be marked as unknown later
				},
			},
			wantErr: false,
		},
		{
			name: "pass when current version is v1alpha3 contract, next v1alpha4",
			fields: fields{
				reader: test.NewFakeReader().
					WithProvider("p1", clusterctlv1.InfrastructureProviderType, "https://somewhere.com"),
		...
		{
			name: "fails if a metadata file for upgrades can't be find",
			fields: fields{
				reader: test.NewFakeReader().
					WithProvider("p1", clusterctlv1.InfrastructureProviderType, "https://somewhere.com"),
				repository: test.NewFakeRepository(). //without metadata
