...
func (r *Reconciler) reconcileExternal(ctx context.Context, cluster *clusterv1.Cluster, ref *clusterv1.ObjectReference) (*unstructured.Unstructured, error) {
	log := ctrl.LoggerFrom(ctx)

	obj, err := external.GetObjectFromRef(ctx, r.Client, ref, cluster.Namespace)
	if err != nil {
		return nil, err
	}
