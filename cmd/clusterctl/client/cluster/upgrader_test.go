...
func Test_providerUpgrader_checkClusterClassRefs(t *testing.T) {
	...
	infraRepoWithCRD := func(crd *apiextensionsv1.CustomResourceDefinition) *repository.MemoryRepository {
		return repository.NewMemoryRepository().
			WithPaths("", "components.yaml").
			WithVersions("v2.0.0", "v2.0.1").
			WithFile("v2.0.1", "components.yaml", crdToYAML(crd)).
			WithMetadata("v2.0.1", &clusterctlv1.Metadata{
				ReleaseSeries: []clusterctlv1.ReleaseSeries{
					{Major: 2, Minor: 0, Contract: currentContractVersion},
				},
	...
	infraRepo := func(newVersions ...string) *repository.MemoryRepository {
		return infraRepoWithCRD(fakeCRD(crdName, infraGroup, infraKind, newVersions...))
	}

	infraRepoWithServedV1beta2UnservedV1beta1 := func() *repository.MemoryRepository {
		crd := fakeCRD(crdName, infraGroup, infraKind, "v1beta1", "v1beta2")
		markUnserved(crd, "v1beta1")
		return infraRepoWithCRD(crd)
	}
	...
	ccWithInfraTemplateRefAPIVersion := func(apiVersion string) *clusterv1.ClusterClass {
	...
	tests := []struct {
		...
		},
		{
			...
			proxy: test.NewFakeProxy().
				...
				WithObjs(
					...
					ccWithInfraTemplateRefAPIVersion(infraGroup+"/v1beta1"),
				),
			plan: infraPlan,
			wantErr: "upgrade pre-flight check failed:\n" +
				"- upgrading infra-system/infrastructure-infra would stop serving apiVersion \"v1beta1\" of " +
				"infrastructure.cluster.x-k8s.io/TestInfraClusterTemplate, which is still referenced by ClusterClass(es) " +
				"[default/cc1]; update the ClusterClass templateRefs to a served apiVersion before upgrading",
		},
		{
			name: "version unserved, ClusterClass pins unserved version: error",
			repo: infraRepoWithServedV1beta2UnservedV1beta1(),
			proxy: test.NewFakeProxy().
				WithProviderInventory("infra", clusterctlv1.InfrastructureProviderType, "v2.0.0", "infra-system").
				WithObjs(
					fakeCRD(crdName, infraGroup, infraKind, "v1beta1", "v1beta2"),
		...
		},
		{
			...
			proxy: test.NewFakeProxy().
				...
				WithObjs(
					fakeCRD(crdName, infraGroup, infraKind, "v1beta1", "v1beta2"),
					ccWithInfraTemplateRefAPIVersion(infraGroup+"/v1beta2"),
				),
			plan: infraPlan,
		},
	}
...
func markUnserved(crd *apiextensionsv1.CustomResourceDefinition, version string) {
...
func crdToYAML(crd *apiextensionsv1.CustomResourceDefinition) []byte {
