...
func validateCanUpdateMachineRequests(currentMachine *clusterv1.Machine, machineUpToDateResult pkg.UpToDateResult, callExtensionExpectedChanges map[string]func(runtime.Object)) func(name string, object runtimehooksv1.RequestObject) error {
	return func(name string, req runtimehooksv1.RequestObject) error {
		...
		case *runtimehooksv1.CanUpdateMachineRequest:
			...
			admission.ApplyPreviousKubeadmConfigDefaults(&currentKubeadmConfig.Spec) // PrepareKubeadmConfigsForDiff applies defaults.
			if mutator, ok := callExtensionExpectedChanges[name]; ok {
			...
			admission.ApplyPreviousKubeadmConfigDefaults(&desiredKubeadmConfig.Spec) // PrepareKubeadmConfigsForDiff applies defaults.
...
func Test_createRequest(t *testing.T) {
	...
	admission.ApplyPreviousKubeadmConfigDefaults(&currentKubeadmConfigCleanedUp.Spec)                     // PrepareKubeadmConfigsForDiff applies defaults.
	...
	admission.ApplyPreviousKubeadmConfigDefaults(&desiredKubeadmConfigCleanedUp.Spec)                     // PrepareKubeadmConfigsForDiff applies defaults.
