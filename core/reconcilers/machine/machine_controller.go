...
func (r *Reconciler) reconcileDelete(ctx context.Context, s *scope) (ctrl.Result, error) { //nolint:gocyclo
	...
	if isDeleteNodeAllowed {
		...
		// In case the preTerminateHook is started or the infra machine is not found the Node drain is skipped.
		...
		// In case the node is unreachable, preTerminateHook is started or the infra machine is not found the wait for volume detachment is skipped.
...
func (r *Reconciler) reconcileDeleteBootstrap(ctx context.Context, s *scope) (bool, error) {
	...
	if s.bootstrapConfig != nil && s.bootstrapConfig.GetDeletionTimestamp().IsZero() {
		if err := r.Client.Delete(ctx, external.ToUnstructured(s.bootstrapConfigGVK, s.bootstrapConfig)); err != nil && !apierrors.IsNotFound(err) {
...
func (r *Reconciler) reconcileDeleteInfrastructure(ctx context.Context, s *scope) (bool, error) {
	...
	if s.infraMachine != nil && s.infraMachine.GetDeletionTimestamp().IsZero() {
		if err := r.Client.Delete(ctx, external.ToUnstructured(s.infraMachineGVK, s.infraMachine)); err != nil && !apierrors.IsNotFound(err) {
			return false, pkgerrors.Wrapf(err,
				"failed to delete %v %q for Machine %q in namespace %q",
