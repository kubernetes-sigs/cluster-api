...
func ComputeKubeadmConfig(kcp *controlplanev1.KubeadmControlPlane, cluster *clusterv1.Cluster, isJoin bool, name string, existingKubeadmConfig *bootstrapv1.KubeadmConfig) (*bootstrapv1.KubeadmConfig, error) {
	...
	if isJoin {
		spec = joinControlPlaneConfig(kcp)
	} else {
		spec = initialControlPlaneConfig(kcp)
	}

	parsedVersion, err := semver.ParseTolerant(kcp.Spec.Version)
	if err != nil {
		return nil, errors.Wrapf(err, "failed to parse kubernetes version %q", kcp.Spec.Version)
	}
...
func initialControlPlaneConfig(kcp *controlplanev1.KubeadmControlPlane) *bootstrapv1.KubeadmConfigSpec {
...
func joinControlPlaneConfig(kcp *controlplanev1.KubeadmControlPlane) *bootstrapv1.KubeadmConfigSpec {
	bootstrapSpec := kcp.Spec.KubeadmConfigSpec.DeepCopy()
...
func ComputeInfraMachine(ctx context.Context, c client.Client, kcp *controlplanev1.KubeadmControlPlane, cluster *clusterv1.Cluster, name string, existingInfraMachine *unstructured.Unstructured) (*unstructured.Unstructured, error) {
	...
	from, err := external.Get(ctx, c, templateRef)
	if err != nil {
		return nil, err
	}
