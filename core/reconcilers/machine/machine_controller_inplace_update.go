...
func (r *Reconciler) callUpdateMachineHook(ctx context.Context, s *scope) (ctrl.Result, string, error) {
	...
	infraMachineCacheEntry, ok := r.contractObjectsCache.Has(contractObjectsCacheEntryFromContractObject(s.infraMachineGVK, s.infraMachine))
	...
	if s.bootstrapConfig != nil {
		bootstrapConfigCacheEntry, ok = r.contractObjectsCache.Has(contractObjectsCacheEntryFromContractObject(s.bootstrapConfigGVK, s.bootstrapConfig))
...
func (r *Reconciler) completeInPlaceUpdate(ctx context.Context, s *scope) error {
	...
	if s.infraMachine == nil {
		...
		c, err := r.DynamicCache.GetWriter(ctx, s.infraMachineGVK, s.infraMachine)
		if err != nil {
			return pkgerrors.Wrapf(err, "failed to remove %s annotation from object %s: failed to get client", clusterv1.UpdateInProgressAnnotation, klog.KObj(s.infraMachine))
		}
	...
	if s.bootstrapConfig != nil {
		c, err := r.DynamicCache.GetWriter(ctx, s.bootstrapConfigGVK, s.bootstrapConfig)
		if err != nil {
			return pkgerrors.Wrapf(err, "failed to remove %s annotation from object %s: failed to get client", clusterv1.UpdateInProgressAnnotation, klog.KObj(s.bootstrapConfig))
		}
...
// contractObjectsCacheEntry is an Entry for the contractObjects cache.
type contractObjectsCacheEntry struct {
	obj *unstructured.Unstructured
}

// newContractObjectsCacheEntry creates a new contractObjectsCacheEntry.
func newContractObjectsCacheEntry(obj *unstructured.Unstructured) contractObjectsCacheEntry {
	return contractObjectsCacheEntry{
		obj: obj,
	}
}
...
func contractObjectsCacheEntryFromContractObject(gvk schema.GroupVersionKind, obj client.Object) string {
	return fmt.Sprintf("%s: %s", gvk, klog.KObj(obj))
}
