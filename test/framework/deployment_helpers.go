...
type Metadata struct {
	...
	// FIXME: cluster/machine? (per log line)
}
...
func WatchDeploymentLogs(ctx context.Context, input WatchDeploymentLogsInput) {
	...
	for _, pod := range pods.Items {
		for _, container := range deployment.Spec.Template.Spec.Containers {
			...
			logMetadataFile := filepath.Clean(path.Join(input.LogPath, input.Deployment.Name, pod.Name, container.Name+".json"))
			Expect(os.MkdirAll(filepath.Dir(logMetadataFile), 0750)).To(Succeed())

			// FIXME: verify where portal takes the data from
			metadata := Metadata{
