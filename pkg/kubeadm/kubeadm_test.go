...
func TestTokenCreateParameters(t *testing.T) {
	...
	kadm := kubeadm.NewWithCmdRunner(testcmdrunner.NewTestRunnerFailOnErr(t, echoCallback))
...
func TestTokenCreateReturnsUnmodifiedOutput(t *testing.T) {
	kadm := kubeadm.NewWithCmdRunner(testcmdrunner.NewTestRunnerFailOnErr(t, tokenCallback))
...
func TestNonZeroExitCodeResultsInError(t *testing.T) {
	kadm := kubeadm.NewWithCmdRunner(testcmdrunner.NewTestRunnerFailOnErr(t, errorCallback))
	output, err := kadm.TokenCreate(kubeadm.TokenCreateParams{})
	if err == nil {
