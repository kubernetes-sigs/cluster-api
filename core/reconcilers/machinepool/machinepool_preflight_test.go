...
func TestMachinePoolReconciler_runPreflightChecks(t *testing.T) {
	...
	t.Run("should run preflight checks if the feature gate is enabled", func(t *testing.T) {
		tests := []struct {
			...
			{
				name: "kubeadm version preflight check: should fail if the machine pool major version differs from the control plane while the minor version matches when using kubeadm bootstrap provider",
				cluster: &clusterv1.Cluster{
					ObjectMeta: metav1.ObjectMeta{Namespace: ns},
					Spec: clusterv1.ClusterSpec{
						ControlPlaneRef: contract.ObjToContractVersionedObjectReference(controlPlaneStable),
				...
				wantMessages: []string{
					"MachinePool version (2.26.0) and ControlPlane version (1.26.2) do not conform to kubeadm version skew policy as kubeadm only supports joining with the same major+minor version as the control plane (\"KubeadmVersionSkew\" preflight check failed)",
				},
				wantErr: false,
			},
			{
