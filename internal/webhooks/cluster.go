...
func (webhook *Cluster) validateTopologyVersion(ctx context.Context, fldPath *field.Path, fldValue string, inVersion, oldVersion semver.Version, oldCluster *clusterv1.Cluster, shouldValidateCeiling bool) *field.Error {
	...
	if shouldValidateCeiling {
		...
		ceilVersion := semver.Version{
			...
			Minor: oldVersion.Minor + 5, // TODO: FP, fix this, this should consider if there is an actual update plan to the required version (which in turns should ensure version skew constraints are fulfilled)
			Patch: 0,
		}
