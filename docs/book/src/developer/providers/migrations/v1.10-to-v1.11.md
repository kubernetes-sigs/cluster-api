...
- The following fields in `spec.topology.{controlPlane.machineHealthCheck,workers.machineDeployments[].machineHealthCheck` have been renamed: (their type was also changed from Duration to int)
...
- The following fields in `spec.template.spec` have been renamed: (their type was also changed from Duration to int)
  - `nodeDrainTimeout` => `nodeDrainTimeoutSeconds`
  - `nodeVolumeDetachTimeout` => `nodeVolumeDetachTimeoutSeconds`
  - `nodeDeletionTimeout` => `nodeDeletionTimeoutSeconds`
...
- See changes that apply to [all CRDs](#all-crds)
- The following fields in `spec` have been renamed: (their type was also changed from Duration to int)
  - `nodeDrainTimeout` => `nodeDrainTimeoutSeconds`
  - `nodeVolumeDetachTimeout` => `nodeVolumeDetachTimeoutSeconds`
  - `nodeDeletionTimeout` => `nodeDeletionTimeoutSeconds`
...
- The following fields in `spec` have been renamed: (their type was also changed from Duration to int)
  - `nodeStartupTimeout` => `nodeStartupTimeoutSeconds`
- The following fields in `spec.unhealthyNodeConditions[]` have been renamed: (their type was also changed from Duration to int)
...
- The following fields in `spec.{controlPlane,workers.machineDeployments[],workers.machinePools[]` have been renamed: (their type was also changed from Duration to int)
  - `nodeDrainTimeout` => `nodeDrainTimeoutSeconds`
  - `nodeVolumeDetachTimeout` => `nodeVolumeDetachTimeoutSeconds`
  - `nodeDeletionTimeout` => `nodeDeletionTimeoutSeconds`
- The following fields in `spec.{controlPlane.machineHealthCheck,workers.machineDeployments[].machineHealthCheck` have been renamed: (their type was also changed from Duration to int)
  - `nodeStartupTimeout` => `nodeStartupTimeoutSeconds`
  - `unhealthyNodeConditions[].timeout` => `unhealthyNodeConditions[].timeoutSeconds`
...
- The following `spec.kubeadmConfigSpec.clusterConfiguration` fields have been removed:
...
- The `spec.initConfiguration.bootstrapTokens[].ttl` field has been renamed to `ttlSeconds` (type was also changed from Duration to int)
- Information about the initial provisioning process is now surfacing under the new `status.initialization` field.
...
- The following `spec.kubeadmConfigSpec.clusterConfiguration` fields have been removed:
  ...
  Note: The ClusterConfiguration fields could previously be used to overwrite the fields from Cluster, now we only use the fields from Cluster.
- The `spec.initConfiguration.bootstrapTokens[].ttl` field has been renamed to `ttlSeconds` (type was also changed from Duration to int)
- The following fields in `spec.machineTemplate` have been renamed: (their type was also changed from Duration to int)
  - `nodeDrainTimeout` => `nodeDrainTimeoutSeconds`
  - `nodeVolumeDetachTimeout` => `nodeVolumeDetachTimeoutSeconds`
  - `nodeDeletionTimeout` => `nodeDeletionTimeoutSeconds`
- The following fields in `spec.remediationStrategy` have been renamed: (their type was also changed from Duration to int)
