...
const (
	...
	scaleNamespacePlaceholder   = "scale-namespace-placeholder"
)
...
type scaleSpecInput struct {
	...
	// SeparateNamespacePerCluster defines if each cluster should be deployed into its separate namespace.
	// In this case The namespace name will be the name of the cluster.
	SeparateNamespacePerCluster bool
...
func scaleSpec(ctx context.Context, inputGetter func() scaleSpecInput) {
	...
	It("Should create and delete workload clusters", func() {
		...
		baseWorkloadClusterTemplate := clusterctl.ConfigCluster(ctx, clusterctl.ConfigClusterInput{
			...
			Flavor:                   flavor,
			Namespace:                scaleNamespacePlaceholder,
		...
		if !input.SeparateNamespacePerCluster {
			if len(baseClusterClassYAML) > 0 {
				clusterClassYAML := bytes.Replace(baseClusterClassYAML, []byte(scaleNamespacePlaceholder), []byte(namespace.Name), -1)
		...
		clusterCreateResults, err := workConcurrentlyAndWait(ctx, workConcurrentlyAndWaitInput{
			...
			WorkerFunc: func(ctx context.Context, inputChan chan string, resultChan chan workResult, wg *sync.WaitGroup) {
				createClusterWorker(ctx, input.BootstrapClusterProxy, inputChan, resultChan, wg, namespace.Name, input.SeparateNamespacePerCluster, baseClusterClassYAML, baseClusterTemplateYAML, creator)
			},
		})
		if err != nil {
...
func createClusterWorker(ctx context.Context, clusterProxy framework.ClusterProxy, inputChan <-chan string, resultChan chan<- workResult, wg *sync.WaitGroup, defaultNamespace string, separateNamespacePerCluster bool, baseClusterClassYAML, baseClusterTemplateYAML []byte, create clusterCreator) {
	defer wg.Done()
	...
	for {
			...
			case clusterName, open := <-inputChan:
				...
				if separateNamespacePerCluster {
				...
				if separateNamespacePerCluster {
					...
					clusterClassYAML := bytes.Replace(baseClusterClassYAML, []byte(scaleNamespacePlaceholder), []byte(namespaceName), -1)
					Eventually(func() error {
				...
				clusterTemplateYAML := bytes.Replace(baseClusterTemplateYAML, []byte(scaleNamespacePlaceholder), []byte(namespaceName), -1)
