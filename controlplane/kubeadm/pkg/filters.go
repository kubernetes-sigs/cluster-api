...
func UpToDate(
	...
	matches, specLogMessages, specConditionMessages, err := matchesMachineSpec(ctx, dynamicCache, infraMachines, kubeadmConfigs, kcp, cluster, machine, res)
	if err != nil {
...
func matchesMachineSpec(
	...
	reason, currentInfraMachine, desiredInfraMachine, matches, err := matchesInfraMachine(ctx, dynamicCache, infraMachines, kcp, cluster, machine)
	if err != nil {
		return false, nil, nil, pkgerrors.Wrapf(err, "failed to match Machine")
	}
...
func matchesInfraMachine(
	...
	desiredInfraMachine, err := desiredstate.ComputeDesiredInfraMachine(ctx, dynamicCache, kcp, cluster, machine.Name, currentInfraMachine)
	if err != nil {
...
func PrepareKubeadmConfigsForDiff(desiredKubeadmConfig, currentKubeadmConfig *bootstrapv1.KubeadmConfig, convertCurrentInitConfigurationToJoinConfiguration bool) (desired, current *bootstrapv1.KubeadmConfig) {
	...
	bootstrapadmission.ApplyPreviousKubeadmConfigDefaults(&desiredKubeadmConfig.Spec)
	bootstrapadmission.ApplyPreviousKubeadmConfigDefaults(&currentKubeadmConfig.Spec)
