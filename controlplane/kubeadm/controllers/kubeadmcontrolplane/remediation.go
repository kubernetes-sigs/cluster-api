...
import (
	...
	"sigs.k8s.io/cluster-api/controlplane/kubeadm/pkg/etcd/util"
...
func (r *Reconciler) reconcileUnhealthyMachines(ctx context.Context, controlPlane *pkg.ControlPlane) (ret ctrl.Result, retErr error) {
...
func (r *Reconciler) checkRetryLimits(log logr.Logger, machineToBeRemediated *clusterv1.Machine, controlPlane *pkg.ControlPlane, reconciliationTime time.Time) (*RemediationData, bool, error) {
...
func (r *Reconciler) canSafelyRemediateMachine(ctx context.Context, controlPlane *pkg.ControlPlane, machineToBeRemediated *clusterv1.Machine) bool {
	log := ctrl.LoggerFrom(ctx)
...
func (r *Reconciler) tryGetEtcdMemberName(ctx context.Context, controlPlane *pkg.ControlPlane, deletingMachine *clusterv1.Machine) string {
	log := ctrl.LoggerFrom(ctx)
	...
	if providerID != "" {
		...
		if err != nil {
			...
			if nodes, err := pkg.ListTransformedNodes(ctx, remoteClient, client.MatchingFields{index.NodeProviderIDField: providerID}); err == nil {
				if len(nodes) == 1 {
					node = nodes[0].Name
				}
			}
