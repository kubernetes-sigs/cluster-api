...
const (
	// MachineFinalizer allows ReconcileDockerMachine to clean up resources associated with DevMachine before
	// removing it from the apiserver.
	MachineFinalizer = "dockermachine.infrastructure.cluster.x-k8s.io"

	// LoadbalancerWeightAnnotation define the load balancer weight for this machine.
	LoadbalancerWeightAnnotation = "devmachine.infrastructure.cluster.x-k8s.io/weight"
)

// Deprecated v1beta1 conditions and reasons for DevMachine's docker backend.
const (
	...
	// WaitingForClusterInfrastructureV1Beta1Reason documents a DockerMachine waiting for the cluster
	// infrastructure to be ready before starting to create the container.
	//
	...
	// WaitingForBootstrapDataV1Beta1Reason documents a DockerMachine waiting for the bootstrap
	// script to be ready before starting to create the container.
	//
	// Deprecated: This reason is deprecated and is going to be removed when support for v1beta1 is dropped.
	...
	// BootstrappingV1Beta1Reason documents a DockerMachine currently executing the bootstrap script.
	//
	// Deprecated: This reason is deprecated and is going to be removed when support for v1beta1 is dropped.
	BootstrappingV1Beta1Reason = "Bootstrapping"

	// BootstrapFailedV1Beta1Reason documents a DockerMachine controller detecting an error while bootstrapping.
	//
	// Deprecated: This reason is deprecated and is going to be removed when support for v1beta1 is dropped.
...
const (
	...
	// - The DevMachine's is using a docker backend and CGroupReady, PreLoadedImagesReady, CloudInitOrIgnitionCompleted conditions are true.
...
// CGroupReady condition and corresponding reasons for a DevMachine's docker backend.
const (
	// DevMachineDockerCGroupsReadyCondition documents the status of cgroups inside the container
	// generated by a DevMachine's docker backend.
	DevMachineDockerCGroupsReadyCondition string = "CGroupsReady"

	// DevMachineDockerCGroupsReadyWaitingForContainerReason documents when the system is waiting for the creation of
	// the container for a DevMachine's docker backend.
	DevMachineDockerCGroupsReadyWaitingForContainerReason = "WaitingForContainer"

	// DevMachineDockerCGroupsReadyReason documents when cgroups inside the container for a DevMachine's docker backend
	// are ready.
	DevMachineDockerCGroupsReadyReason = clusterv1.ReadyReason

	// DevMachineDockerCGroupsNotReadyReason documents when cgroups inside the container for a DevMachine's docker backend
	// are not ready.
	DevMachineDockerCGroupsNotReadyReason = clusterv1.NotReadyReason

	// DevMachineDockerCGroupsReadyInternalErrorReason documents an unexpected error happened when waiting
	// cgroups to be ready in the container for a DevMachine's docker backend are not ready.
	DevMachineDockerCGroupsReadyInternalErrorReason = clusterv1.InternalErrorReason
)

// PreLoadedImagesReady condition and corresponding reasons for a DevMachine's docker backend.
const (
	// DevMachineDockerPreLoadedImagesReadyCondition documents the status of preloaded images inside the container
	// generated by a DevMachine's docker backend.
	DevMachineDockerPreLoadedImagesReadyCondition string = "PreLoadedImagesReady"

	// DevMachineDockerPreLoadedImagesReadyWaitingForContainerReason documents when the system is waiting for the creation of
	// the container for a DevMachine's docker backend.
	DevMachineDockerPreLoadedImagesReadyWaitingForContainerReason = "WaitingForContainer"

	// DevMachineDockerPreLoadedImagesReadyWaitingForCGroupsReason documents when the system is waiting for
	// cgroups inside the container for a DevMachine's docker backend.
	DevMachineDockerPreLoadedImagesReadyWaitingForCGroupsReason = "WaitingForCGroups"

	// DevMachineDockerPreLoadedImagesReadyReason documents when preloaded images inside the container for a DevMachine's docker backend
	// are ready.
	DevMachineDockerPreLoadedImagesReadyReason = clusterv1.ReadyReason

	// DevMachineDockerPreLoadedImagesNotReadyReason documents when preloaded images inside the container for a DevMachine's docker backend
	// are not ready.
	DevMachineDockerPreLoadedImagesNotReadyReason = clusterv1.NotReadyReason

	// DevMachineDockerPreLoadedImagesReadyInternalErrorReason documents an unexpected error happened when
	// preloading images inside the container for a DevMachine's docker backend.
	DevMachineDockerPreLoadedImagesReadyInternalErrorReason = clusterv1.InternalErrorReason
)

// BootstrapCompleted condition and corresponding reasons for a DevMachine's docker backend.
const (
	// DevMachineBootstrapCompletedCondition documents the progress of the bootstrap
	// script executed after the provisioning of the container generated by a DevMachine's docker backend.
	DevMachineBootstrapCompletedCondition string = "BootstrapCompleted"

	// DevMachineDockerBootstrapCompletedWaitingForContainerReason documents when the system is waiting for the creation of
	// the container for a DevMachine's docker backend.
	DevMachineDockerBootstrapCompletedWaitingForContainerReason = "WaitingForContainer"

	// DevMachineDockerBootstrapCompletedWaitingForCGroupsReason documents when the system is waiting for
	// cgroups inside the container for a DevMachine's docker backend.
	DevMachineDockerBootstrapCompletedWaitingForCGroupsReason = "WaitingForCGroups"

	// DevMachineDockerBootstrapCompletedWaitingForPreloadedImagesReason documents when the system is waiting for
	// preloaded images inside the container for a DevMachine's docker backend.
	DevMachineDockerBootstrapCompletedWaitingForPreloadedImagesReason = "WaitingForPreloadedImages"

	// DevMachineDockerBootstrapCompletedReason documents when preloaded the bootstrap
	// script executed inside the container for a DevMachine's docker backend is completed.
	DevMachineDockerBootstrapCompletedReason = "Completed"

	// DevMachineDockerBootstrapNotCompletedReason documents when preloaded the bootstrap
	// script executed inside the container for a DevMachine's docker backend is not completed.
	DevMachineDockerBootstrapNotCompletedReason = "NotCompleted"

	// DevMachineDockerBootstrapFailedReason documents when preloaded the bootstrap
	// script executed inside the container for a DevMachine's docker backend failed.
	DevMachineDockerBootstrapFailedReason = "Failed"

	// DevMachineDockerBootstrapCompletedInternalErrorReason documents an unexpected error happened when
	// executing the bootstrap script inside the container for a DevMachine's docker backend.
	DevMachineDockerBootstrapCompletedInternalErrorReason = clusterv1.InternalErrorReason
)
...
type DevMachineStatus struct {
	...
	// - Ready, VMProvisioned, NodeProvisioned, EtcdProvisioned, APIServerProvisioned and Paused when using the in-memory backend.
	// - Ready, ContainerProvisioned, CGroupsReady, PreLoadedImagesReady, BoostrapCompleted and Paused when using the docker backend.
	// +optional
	// +listType=map
	// +listMapKey=type
