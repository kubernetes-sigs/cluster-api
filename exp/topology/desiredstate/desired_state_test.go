...
func TestComputeMachineDeploymentVersion(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// A version that is set is always honored, the feature gate only controls if it can be set.
			name:                          "should return the pinned version even if the feature gate is disabled",
...
func TestPinnedWorkerNames(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// A version that is set is always honored, the feature gate only controls if it can be set.
			name:               "returns pinned MachineDeployments even if the feature gate is disabled",
			versionPinning:     false,
			machineDeployments: map[string]string{"md-topology-1": "v1.31.0"},
			currentObjects:     map[string]string{"md-topology-1": "md-1-obj"},
			want:               []string{"md-1-obj"},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
