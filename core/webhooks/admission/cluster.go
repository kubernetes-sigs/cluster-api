...
func validateWorkerVersion(pinnedVersion string, annotations map[string]string, topologyVersion string, fldPath *field.Path) field.ErrorList {
	if pinnedVersion == "" {
		return nil
	}
	...
	pinnedSemVer, pinnedErr := semver.Parse(strings.TrimPrefix(pinnedVersion, "v"))
	if pinnedErr != nil {
		allErrs = append(allErrs, field.Invalid(fldPath, pinnedVersion, "must be a valid semantic version"))
	}

	// The pinned version is bounded by the version the Cluster manages the control plane to, because
	// a pinned MachineDeployment/MachinePool is not upgraded by the cluster-level rollout. These are
	// the only bounds on create and for a newly added topology, where there is neither a previous
	// pin nor a control plane object to compare against.
	...
	if pinnedErr == nil && topologyErr == nil {
		switch {
		case version.Compare(pinnedSemVer, topologySemVer, version.WithoutPreReleases()) > 0:
			allErrs = append(allErrs, field.Invalid(fldPath, pinnedVersion,
				fmt.Sprintf("version cannot be greater than Cluster.spec.topology.version %q", topologyVersion)))
		case !version.WorkerVersionSkewSupported(topologySemVer, pinnedSemVer):
			allErrs = append(allErrs, field.Invalid(fldPath, pinnedVersion,
