...
func (src *Cluster) ConvertTo(dstRaw conversion.Hub) error {
	...
	if src.Spec.InfrastructureRef != nil {
		gv, err := schema.ParseGroupVersion(src.Spec.InfrastructureRef.APIVersion)
		if err != nil {
			return fmt.Errorf("failed to convert object: failed to parse apiVersion from Cluster.spec.infrastructureRef: %v", err)
		}
		dst.Spec.InfrastructureRef = &clusterv1.ObjectReference{
			APIGroup: gv.Group,
			Kind:     src.Spec.InfrastructureRef.Kind,
			Name:     src.Spec.InfrastructureRef.Name,
		}
	}

	if src.Spec.ControlPlaneRef != nil {
		gv, err := schema.ParseGroupVersion(src.Spec.ControlPlaneRef.APIVersion)
		if err != nil {
			return fmt.Errorf("failed to convert object: failed to parse apiVersion from Cluster.spec.controlPlaneRef: %v", err)
		}
		dst.Spec.ControlPlaneRef = &clusterv1.ObjectReference{
			APIGroup: gv.Group,
			Kind:     src.Spec.ControlPlaneRef.Kind,
			Name:     src.Spec.ControlPlaneRef.Name,
		}
	}
...
func (dst *Cluster) ConvertFrom(srcRaw conversion.Hub) error {
	...
	if src.Spec.InfrastructureRef != nil {
		apiVersion, err := apiVersionGetter(schema.GroupKind{
			Group: src.Spec.InfrastructureRef.APIGroup,
			Kind:  src.Spec.InfrastructureRef.Kind,
		})
		if err != nil {
			return fmt.Errorf("failed to convert object: %v", err)
		}
	...
	if src.Spec.ControlPlaneRef != nil {
		apiVersion, err := apiVersionGetter(schema.GroupKind{
			Group: src.Spec.ControlPlaneRef.APIGroup,
			Kind:  src.Spec.ControlPlaneRef.Kind,
		})
		if err != nil {
			return fmt.Errorf("failed to convert object: %v", err)
		}
		dst.Spec.ControlPlaneRef = &corev1.ObjectReference{
			APIVersion: apiVersion,
			Kind:       src.Spec.ControlPlaneRef.Kind,
			Namespace:  src.Namespace,
			Name:       src.Spec.ControlPlaneRef.Name,
...
func Convert_v1_ObjectReference_To_v1beta2_ObjectReference(_ *corev1.ObjectReference, _ *clusterv1.ObjectReference, _ apimachineryconversion.Scope) error {
	// This is implemented in ConvertTo/ConvertFrom as we have all necessary information available there.
	return nil
}

func Convert_v1beta2_ObjectReference_To_v1_ObjectReference(_ *clusterv1.ObjectReference, _ *corev1.ObjectReference, _ apimachineryconversion.Scope) error {
	// This is implemented in ConvertTo/ConvertFrom as we have all necessary information available there.
	return nil
}
