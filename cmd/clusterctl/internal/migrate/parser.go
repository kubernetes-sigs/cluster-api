...
// YAMLParser implements the Parser interface using Kubernetes runtime serializers.
	...
	for i, doc := range docs {
		...
		if doc.Object != nil {
			...
			if err := p.yamlSerializer.Encode(doc.Object, buf); err != nil {
				return fmt.Errorf("failed to encode document at index %d: %w", doc.Index, err)
			}
			if _, err := writer.Write(p.ensureTrailingNewline(buf.Bytes())); err != nil {
				return fmt.Errorf("failed to write document at index %d: %w", doc.Index, err)
			}
			continue
		}

		if _, err := writer.Write(p.ensureTrailingNewline(doc.Raw)); err != nil {
			return fmt.Errorf("failed to write raw document at index %d: %w", doc.Index, err)
		}
	}
...
func (p *YAMLParser) parseDocument(trimmed []byte, original []byte, index int) (Document, error) {
	...
	if err != nil || gvk == nil || gvk.Empty() {
		return Document{}, fmt.Errorf("unable to decode document")
	}
