...
func (r *Reconciler) reconcileControlPlane(ctx context.Context, s *scope.Scope) (bool, error) {
	...
	if s.Blueprint.HasControlPlaneInfrastructureMachine() {
		...
		if err != nil {
			return false, errors.Wrapf(err, "failed to reconcile %s", klog.KObj(s.Desired.ControlPlane.InfrastructureMachineTemplate))
		}
		...
		if createdInfrastructureTemplate {
			infrastructureMachineCleanupFunc = func() {
				...
				if err := r.Client.Delete(ctx, s.Desired.ControlPlane.InfrastructureMachineTemplate); err != nil {
					...
					log.Info("WARNING! Failed to cleanup InfrastructureMachineTemplate for control plane while handling creation or update error. The object will be garbage collected when the cluster is deleted.")
				}
			}
		}
		...
		if err != nil {
			...
			return false, errors.Wrapf(err, "failed to reconcile %s", klog.KObj(s.Desired.ControlPlane.Object))
		}
	}
...
func (r *Reconciler) reconcileCluster(ctx context.Context, s *scope.Scope) error {
	...
	if len(changes) == 0 {
		...
		log.Info(fmt.Sprintf("Patching Cluster, diff: %s", changes))
	}
	if err := patchHelper.Patch(ctx); err != nil {
...
func (r *Reconciler) createMachineDeployment(ctx context.Context, s *scope.Scope, md *scope.MachineDeploymentState) error {
	...
	log := ctrl.LoggerFrom(ctx).WithValues(
		...
		"machineDeploymentTopology", mdTopologyName)
	cluster := s.Current.Cluster
	infraCtx := ctrl.LoggerInto(ctx, log.WithValues(md.InfrastructureMachineTemplate.GetKind(), klog.KObj(md.InfrastructureMachineTemplate)))
	infrastructureMachineCleanupFunc := func() {}
		...
		infrastructureMachineCleanupFunc = func() {
			...
			if err := r.Client.Delete(ctx, md.InfrastructureMachineTemplate); err != nil {
				log := ctrl.LoggerFrom(ctx).
					...
					WithValues("err", err.Error())
				log.Info("WARNING! Failed to cleanup InfrastructureMachineTemplate for MachineDeployment while handling creation error. The object will be garbage collected when the cluster is deleted.")
			}
		}
	}

	bootstrapCtx := ctrl.LoggerInto(ctx, log.WithValues(md.BootstrapTemplate.GetKind(), klog.KObj(md.BootstrapTemplate)))
		...
		bootstrapCleanupFunc = func() {
			...
			if err := r.Client.Delete(ctx, md.BootstrapTemplate); err != nil {
				log := ctrl.LoggerFrom(ctx).
					...
					WithValues("err", err.Error())
				log.Info("WARNING! Failed to cleanup BootstrapTemplate for MachineDeployment while handling creation error. The object will be garbage collected when the cluster is deleted.")
			}
		}
	}

	log = log.WithValues("MachineDeployment", klog.KObj(md.Object))
...
func (r *Reconciler) updateMachineDeployment(ctx context.Context, s *scope.Scope, mdTopologyName string, currentMD, desiredMD *scope.MachineDeploymentState) error {
	...
	cluster := s.Current.Cluster
	infraCtx := ctrl.LoggerInto(ctx, log.WithValues(desiredMD.InfrastructureMachineTemplate.GetKind(), klog.KObj(desiredMD.InfrastructureMachineTemplate)))
	infrastructureMachineCleanupFunc := func() {}
	createdInfra, err := r.reconcileReferencedTemplate(infraCtx, reconcileReferencedTemplateInput{
		cluster:              cluster,
		...
		infrastructureMachineCleanupFunc = func() {
			...
			if err := r.Client.Delete(ctx, desiredMD.InfrastructureMachineTemplate); err != nil {
				log := ctrl.LoggerFrom(ctx).
					...
					WithValues("err", err.Error())
				log.Info("WARNING! Failed to cleanup InfrastructureMachineTemplate for MachineDeployment while handling update error. The object will be garbage collected when the cluster is deleted.")
			}
		}
	}

	bootstrapCtx := ctrl.LoggerInto(ctx, log.WithValues(desiredMD.BootstrapTemplate.GetKind(), klog.KObj(desiredMD.BootstrapTemplate)))
	bootstrapCleanupFunc := func() {}
	createdBootstrap, err := r.reconcileReferencedTemplate(bootstrapCtx, reconcileReferencedTemplateInput{
		cluster:              cluster,
		...
		bootstrapCleanupFunc = func() {
			...
			if err := r.Client.Delete(ctx, desiredMD.BootstrapTemplate); err != nil {
				log := ctrl.LoggerFrom(ctx).
					...
					WithValues("err", err.Error())
				log.Info("WARNING! Failed to cleanup BootstrapTemplate for MachineDeployment while handling update error. The object will be garbage collected when the cluster is deleted.")
			}
		}
	}
	...
	if len(changes) == 0 {
		...
		log.Info(fmt.Sprintf("Patching MachineDeployment, diff: %s", changes))
	}
	if err := patchHelper.Patch(ctx); err != nil {
		// Best effort cleanup of the InfrastructureMachineTemplate & BootstrapTemplate (only on template rotation).
		infrastructureMachineCleanupFunc()
...
func (r *Reconciler) createMachinePool(ctx context.Context, s *scope.Scope, mp *scope.MachinePoolState) error {
	...
	log := ctrl.LoggerFrom(ctx).WithValues(
		...
		"machinePoolTopology", mpTopologyName)
	cluster := s.Current.Cluster
	infraCtx := ctrl.LoggerInto(ctx, log.WithValues(mp.InfrastructureMachinePoolObject.GetKind(), klog.KObj(mp.InfrastructureMachinePoolObject)))
		...
		infrastructureMachineMachinePoolCleanupFunc = func() {
			...
			if err := r.Client.Delete(ctx, mp.InfrastructureMachinePoolObject); err != nil {
				log := ctrl.LoggerFrom(ctx).
					...
					WithValues("err", err.Error())
				log.Info("WARNING! Failed to cleanup InfrastructureMachinePoolObject for MachinePool while handling creation error. The object will be garbage collected when the cluster is deleted.")
			}
		}
	}

	bootstrapCtx := ctrl.LoggerInto(ctx, log.WithValues(mp.BootstrapObject.GetKind(), klog.KObj(mp.BootstrapObject)))
	bootstrapCleanupFunc := func() {}
		...
		bootstrapCleanupFunc = func() {
			...
			if err := r.Client.Delete(ctx, mp.BootstrapObject); err != nil {
				log := ctrl.LoggerFrom(ctx).
					...
					WithValues("err", err.Error())
				log.Info("WARNING! Failed to cleanup BootstrapObject for MachinePool while handling creation error. The object will be garbage collected when the cluster is deleted.")
			}
		}
	}

	log = log.WithValues("MachinePool", klog.KObj(mp.Object))
	...
	r.recorder.Eventf(cluster, corev1.EventTypeNormal, createEventReason, "Created %q", klog.KObj(mp.Object))
...
func (r *Reconciler) updateMachinePool(ctx context.Context, s *scope.Scope, mpTopologyName string, currentMP, desiredMP *scope.MachinePoolState) error {
	...
	if _, err := r.reconcileReferencedObject(infraCtx, reconcileReferencedObjectInput{
		...
		return errors.Wrapf(err, "failed to reconcile %s", klog.KObj(currentMP.Object))
	}
	...
	if err != nil {
		return errors.Wrapf(err, "failed to create patch helper for %s", klog.KObj(currentMP.Object))
	}
	if !patchHelper.HasChanges() {
	...
	if len(changes) == 0 {
		...
		log.Info(fmt.Sprintf("Patching MachinePool, diff: %s", changes))
	}
	if err := patchHelper.Patch(ctx); err != nil {
		return errors.Wrapf(err, "failed to patch %s", klog.KObj(currentMP.Object))
	}
	r.recorder.Eventf(cluster, corev1.EventTypeNormal, updateEventReason, "Updated %q%s", klog.KObj(currentMP.Object), logMachinePoolVersionChange(currentMP.Object, desiredMP.Object))
...
func (r *Reconciler) deleteMachinePool(ctx context.Context, cluster *clusterv1.Cluster, mp *scope.MachinePoolState) error {
	...
	if err := r.Client.Delete(ctx, mp.Object); err != nil && !apierrors.IsNotFound(err) {
		return errors.Wrapf(err, "failed to delete %s", klog.KObj(mp.Object))
	}
	r.recorder.Eventf(cluster, corev1.EventTypeNormal, deleteEventReason, "Deleted %q", klog.KObj(mp.Object))
	return nil
}
...
func (r *Reconciler) reconcileReferencedObject(ctx context.Context, in reconcileReferencedObjectInput) (bool, error) {
	...
	if err != nil {
		return false, errors.Wrapf(err, "failed to create patch helper for %s", klog.KObj(in.current))
	}
	if !patchHelper.HasChanges() {
	...
	if len(changes) == 0 {
		...
		log.Info(fmt.Sprintf("Patching %s, diff: %s", in.desired.GetKind(), changes))
	}
	if err := patchHelper.Patch(ctx); err != nil {
		return false, errors.Wrapf(err, "failed to patch %s", klog.KObj(in.current))
	}
...
func (r *Reconciler) reconcileReferencedTemplate(ctx context.Context, in reconcileReferencedTemplateInput) (bool, error) {
	...
	if !patchHelper.HasSpecChanges() {
		...
		if err := patchHelper.Patch(ctx); err != nil {
			return false, errors.Wrapf(err, "failed to patch %s", klog.KObj(in.desired))
		}
	...
	if len(changes) == 0 {
		...
		log.Info(fmt.Sprintf("Rotating %s, new name %s, diff: %s", in.current.GetKind(), newName, changes))
	}
