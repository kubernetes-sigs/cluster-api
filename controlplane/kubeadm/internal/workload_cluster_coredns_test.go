...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := w.UpdateCoreDNS(context.Background(), tt.kcp, semver.MustParse("1.19.1"))
			if tt.expectErr {
...
func TestUpdateCoreDNSClusterRole(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			fakeClient := fake.NewFakeClientWithScheme(scheme.Scheme, cr)
			...
			g.Expect(fakeClient.Get(ctx, ctrlclient.ObjectKey{Name: coreDNSClusterRoleName, Namespace: metav1.NamespaceSystem}, &actualClusterRole)).To(Succeed())
