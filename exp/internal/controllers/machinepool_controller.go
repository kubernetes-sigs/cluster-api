...
func (r *MachinePoolReconciler) setMachinesUptoDate(ctx context.Context, s *scope) (ctrl.Result, error) {
	...
	for _, machine := range s.machines {
		...
		if err != nil {
			errs = append(errs, errors.Wrapf(err, "failed to create patch helper for machine %q in namespace %q", machine.GetName(), machine.GetNamespace()))
			continue
		}
		...
		if err := patchHelper.Patch(ctx, machine, patch.WithOwnedConditions{Conditions: []string{
			...
			errs = append(errs, errors.Wrapf(err, "failed to patch machine %q in namespace %q", machine.GetName(), machine.GetNamespace()))
			continue
		}
	}
