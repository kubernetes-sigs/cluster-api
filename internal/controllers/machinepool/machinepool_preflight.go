...
func (r *Reconciler) runPreflightChecks(ctx context.Context, cluster *clusterv1.Cluster, mp *clusterv1.MachinePool, action string) ([]string, error) {
	...
	skipped := skippedPreflightChecks(mp)
	...
	if mp.Spec.Template.Spec.Version != "" {
		msVersion := mp.Spec.Template.Spec.Version
		msSemver, err := semver.ParseTolerant(msVersion)
		if err != nil {
			return nil, errors.Wrapf(err, "failed to perform %q: failed to perform preflight checks: failed to parse version %q of MachinePool %s", action, msVersion, klog.KObj(mp))
		}
		...
		if shouldRun(r.PreflightChecks, skipped, clusterv1.MachineSetPreflightCheckKubeadmVersionSkew) {
			if preflightCheckErr := r.kubeadmVersionPreflightCheck(cpSemver, msSemver, mp); preflightCheckErr != nil {
				preflightCheckErrs = append(preflightCheckErrs, preflightCheckErr)
			}
		}
...
func (r *Reconciler) kubernetesVersionPreflightCheck(cpSemver, msSemver semver.Version) preflightCheckErrorMessage {
	...
	if msSemver.Minor > cpSemver.Minor {
		return ptr.To(fmt.Sprintf("MachinePool version (%s) and ControlPlane version (%s) do not conform to the kubernetes version skew policy as MachinePool version is higher than ControlPlane version (%q preflight check failed)", msSemver.String(), cpSemver.String(), clusterv1.MachineSetPreflightCheckKubernetesVersionSkew))
	}
	minorSkew := uint64(3)
	if msSemver.Minor < cpSemver.Minor-minorSkew {
		return ptr.To(fmt.Sprintf("MachinePool version (%s) and ControlPlane version (%s) do not conform to the kubernetes version skew policy as MachinePool version is more than %d minor versions older than the ControlPlane version (%q preflight check failed)", msSemver.String(), cpSemver.String(), minorSkew, clusterv1.MachineSetPreflightCheckKubernetesVersionSkew))
	}

	return nil
}

func (r *Reconciler) kubeadmVersionPreflightCheck(cpSemver, msSemver semver.Version, mp *clusterv1.MachinePool) preflightCheckErrorMessage {
...
func (r *Reconciler) controlPlaneVersionPreflightCheck(cluster *clusterv1.Cluster, cpVersion, msVersion string) preflightCheckErrorMessage {
	if feature.Gates.Enabled(feature.ClusterTopology) && cluster.Spec.Topology.IsDefined() {
		if cpVersion != msVersion {
			return ptr.To(fmt.Sprintf("MachinePool version (%s) is not yet the same as the ControlPlane version (%s), waiting for version to be propagated to the MachinePool (%q preflight check failed)", msVersion, cpVersion, clusterv1.MachineSetPreflightCheckControlPlaneVersionSkew))
		}
	}
...
func skippedPreflightChecks(mp *clusterv1.MachinePool) sets.Set[clusterv1.MachineSetPreflightCheck] {
	skipped := sets.Set[clusterv1.MachineSetPreflightCheck]{}
	if mp == nil {
		return skipped
	}
	skip := mp.Annotations[clusterv1.MachineSetSkipPreflightChecksAnnotation]
	if skip == "" {
		return skipped
	}
