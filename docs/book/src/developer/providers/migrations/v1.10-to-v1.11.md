...
- We highly recommend providers to start using Cluster API v1beta2 types when bumping to CAPI v1.11. 
  ...
  all the all utils the providers is using from an older CAPI release or replace them with something else).
...
because if by mistake you use `sigs.k8s.io/cluster-api/util/patch` in some files this won't lead to compile errors
but conditions are going to be dropped at runtime.
...
- the `deprecatedv1beta1conditions` package alias provides access to utils for managing `clusterv1beta2.Conditions` in `status.deprecated.v1beta1.conditions`
...
- Please pay special attention to use `sigs.k8s.io/cluster-api/util/patch` import everywhere,
  because if by mistake you use `sigs.k8s.io/cluster-api/util/deprecated/v1beta1/patch` in some files this won't lead to compile errors
  but conditions are going to be dropped at runtime.
...
In order to help users transition away from CAPI conditions type, CAPI v1.11 supports
different versions of conditions and patch helper utils.

Following table should help in using the right version
...
Important!
- Please pay special attention to use the correct patch helper import everywhere, because if by mistake you use a wrong
  onw in some files this won't lead to compile errors but conditions are going to be dropped at runtime.
