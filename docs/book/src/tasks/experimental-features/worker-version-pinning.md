...
The feature gate only controls whether `version` can be set or changed. A version that is already
set stays in effect and can be unset even after the feature gate is disabled, so disabling it never
changes what a MachineDeployment/MachinePool runs.
...
- must be a valid semantic version prefixed with `v`, and one of `ClusterClass.spec.kubernetesVersions`,
  if that list is set,
- can never be decreased,
- can never be greater than `Cluster.spec.topology.version`, nor than the version the control plane
  currently runs. The latter matters while a Cluster upgrade is in progress: a pinned
  MachineDeployment/MachinePool rolls out as soon as its version changes, without waiting for the
  control plane, so it can only be raised once the control plane has completed its upgrade,
