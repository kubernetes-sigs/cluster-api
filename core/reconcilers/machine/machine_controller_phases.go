...
func (r *Reconciler) reconcileExternal(ctx context.Context, cluster *clusterv1.Cluster, m *clusterv1.Machine, ref clusterv1.ContractVersionedObjectReference, objectType dynamiccache.ObjectType) (schema.GroupVersionKind, client.Object, error) {
	objGVK, obj, err := r.ensureExternalOwnershipAndWatch(ctx, cluster, m, ref, objectType)
	if err != nil {
		return schema.GroupVersionKind{}, nil, err
	}

	// Set failure reason and message, if any.
	if objF, ok := obj.(interface{ GetFailureReason() string }); ok {
		if failureReason := objF.GetFailureReason(); failureReason != "" {
	...
	if objF, ok := obj.(interface{ GetFailureMessage() string }); ok {
		if failureMessage := objF.GetFailureMessage(); failureMessage != "" {
			if m.Status.Deprecated == nil {
				m.Status.Deprecated = &clusterv1.MachineDeprecatedStatus{}
			}
			if m.Status.Deprecated.V1Beta1 == nil {
			...
			m.Status.Deprecated.V1Beta1.FailureMessage = ptr.To(
				fmt.Sprintf("Failure detected from referenced resource %s %s: %s",
					objGVK.Kind, obj.GetName(), failureMessage),
			)
		}
	}
...
func (r *Reconciler) ensureExternalOwnershipAndWatch(ctx context.Context, cluster *clusterv1.Cluster, m *clusterv1.Machine, ref clusterv1.ContractVersionedObjectReference, objectType dynamiccache.ObjectType) (schema.GroupVersionKind, client.Object, error) {
	...
	hasOnCreateOwnerRefs, err := hasOnCreateOwnerRefs(cluster, m, objGVK, obj)
	if err != nil {
		return schema.GroupVersionKind{}, nil, err
	}
	...
	if err := removeOnCreateOwnerRefs(cluster, m, objGVK, obj); err != nil {
		return schema.GroupVersionKind{}, nil, err
	}
...
func (r *Reconciler) reconcileBootstrap(ctx context.Context, s *scope) (ctrl.Result, error) {
	...
	objGVK, obj, err := r.reconcileExternal(ctx, cluster, m, m.Spec.Bootstrap.ConfigRef, setup.DynamicCacheBootstrapConfigObjectType)
	if err != nil {
	...
	// Note: This type cast is safe because obj will always be a contract BootstrapConfig.
	s.bootstrapConfig = obj.(contractapi.BootstrapConfig)
	...
	contractVersion, err := contract.GetContractVersion(ctx, r.Client, s.bootstrapConfigGVK.GroupKind())
	if err != nil {
		return ctrl.Result{}, err
	}
...
func (r *Reconciler) reconcileInfrastructure(ctx context.Context, s *scope) (ctrl.Result, error) {
	...
	objGVK, obj, err := r.reconcileExternal(ctx, cluster, m, m.Spec.InfrastructureRef, setup.DynamicCacheInfraMachineObjectType)
	if err != nil {
		if apierrors.IsNotFound(err) {
	...
	// Note: This type cast is safe because obj will always be a contract InfraMachine.
	s.infraMachine = obj.(contractapi.InfraMachine)
	s.infraMachineGVK = objGVK

	// Determine contract version used by the InfraMachine.
	contractVersion, err := contract.GetContractVersion(ctx, r.Client, s.infraMachineGVK.GroupKind())
	if err != nil {
		return ctrl.Result{}, err
	}
	...
	m.Status.FailureDomain = s.infraMachine.GetFailureDomain()
...
func removeOnCreateOwnerRefs(cluster *clusterv1.Cluster, m *clusterv1.Machine, objGVK schema.GroupVersionKind, obj client.Object) error {
	cpGK := getControlPlaneGKForMachine(cluster, m)
	for _, owner := range obj.GetOwnerReferences() {
		ownerGV, err := schema.ParseGroupVersion(owner.APIVersion)
		if err != nil {
			return pkgerrors.Wrapf(err, "could not remove ownerReference %v from object %s/%s", owner.String(), objGVK.Kind, obj.GetName())
		}
...
func hasOnCreateOwnerRefs(cluster *clusterv1.Cluster, m *clusterv1.Machine, objGVK schema.GroupVersionKind, obj client.Object) (bool, error) {
	cpGK := getControlPlaneGKForMachine(cluster, m)
	for _, owner := range obj.GetOwnerReferences() {
		ownerGV, err := schema.ParseGroupVersion(owner.APIVersion)
		if err != nil {
			return false, pkgerrors.Wrapf(err, "could not remove ownerReference %v from object %s/%s", owner.String(), objGVK.Kind, obj.GetName())
		}
		if (ownerGV.Group == clusterv1.GroupVersion.Group && owner.Kind == "MachineSet") ||
			(cpGK != nil && ownerGV.Group == cpGK.Group && owner.Kind == cpGK.Kind) {
			return true, nil
