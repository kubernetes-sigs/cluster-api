...
func newConsistencyStore(scheme *runtime.Scheme, cache informerGetter, dynamicCache *external.DynamicCache) *realConsistencyStore { // FIXME: use some interface note *external.DynamicCache
...
func getStore(ctx context.Context, c *realConsistencyStore, gvkt GroupVersionKindType) (kcache.Store, error) {
	...
	if gvkt.Type == ObjectTypeDynamicCacheStructured {
		...
		if !exists {
			return nil, fmt.Errorf("failed to create %s informer: DynamicCache does not have a corresponding Cache yet", gvkt.Kind)
		}
