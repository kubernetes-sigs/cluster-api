...
func Test_canUpdateMachine(t *testing.T) {
	...
	nonEmptyMachineUpToDateResult := pkg.UpToDateResult{
...
func validateCanUpdateMachineRequests(currentMachine *clusterv1.Machine, machineUpToDateResult pkg.UpToDateResult, callExtensionExpectedChanges map[string]func(runtime.Object)) func(name string, object runtimehooksv1.RequestObject) error {
	return func(name string, req runtimehooksv1.RequestObject) error {
		...
		case *runtimehooksv1.CanUpdateMachineRequest:
			...
			kubeadmbootstrapwebhooks.ApplyPreviousKubeadmConfigDefaults(&currentKubeadmConfig.Spec) // PrepareKubeadmConfigsForDiff applies defaults.
			if mutator, ok := callExtensionExpectedChanges[name]; ok {
			...
			kubeadmbootstrapwebhooks.ApplyPreviousKubeadmConfigDefaults(&desiredKubeadmConfig.Spec) // PrepareKubeadmConfigsForDiff applies defaults.
...
func Test_createRequest(t *testing.T) {
	...
	kubeadmbootstrapwebhooks.ApplyPreviousKubeadmConfigDefaults(&currentKubeadmConfigCleanedUp.Spec)      // PrepareKubeadmConfigsForDiff applies defaults.
	...
	kubeadmbootstrapwebhooks.ApplyPreviousKubeadmConfigDefaults(&desiredKubeadmConfigCleanedUp.Spec)      // PrepareKubeadmConfigsForDiff applies defaults.
	...
	tests := []struct {
		...
		modifyUpToDateResult          func(result *pkg.UpToDateResult)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			upToDateResult := pkg.UpToDateResult{
