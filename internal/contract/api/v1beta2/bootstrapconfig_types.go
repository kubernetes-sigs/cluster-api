...
func (c *BootstrapConfig) GetStatusWithConditions() (any, error) {
	status := map[string]any{}
	if len(c.Status.Conditions) > 0 {
		status["conditions"] = c.Status.Conditions.AsAnyArray()
	}
	if c.Status.V1Beta2 != nil && len(c.Status.V1Beta2.Conditions) > 0 {
		status["v1beta2"] = map[string]any{
			"conditions": c.Status.V1Beta2.Conditions.AsAnyArray(),
		}
	}
	return status, nil
}
...
func (c *BootstrapConfig) GetDataSecretCreated() bool {
	return ptr.Deref(c.Status.Initialization.DataSecretCreated, false)
}

func (c *BootstrapConfig) GetDataSecretName() string {
	return c.Status.DataSecretName
}
