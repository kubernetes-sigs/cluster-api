...
- `cluster.{name,namespace,labels,annotations}`
- `cluster.topology.{version,class}`
- `cluster.topology.controlPlane.{labels,annotations,replicas,version}`
    - **Note**: these variables are only available when patching control plane or control plane machine templates.
- `cluster.topology.machineDeployment.current.{labels,annotations,replicas,version,class}`
