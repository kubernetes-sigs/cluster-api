...
func (v *StatusVersions) Get(obj *unstructured.Unstructured) ([]clusterv1.StatusVersion, error) {
	...
	for i, item := range items {
		...
		if !ok {
			return nil, errors.Errorf("failed to parse %s[%d] from object", "."+strings.Join(v.path, "."), i)
		}
		versionValue, ok := itemMap["version"].(string)
		if !ok || versionValue == "" {
			return nil, errors.Errorf("failed to parse %s[%d].version from object", "."+strings.Join(v.path, "."), i)
		}
		statusVersion := clusterv1.StatusVersion{Version: versionValue}
		if replicasValue, ok := itemMap["replicas"]; ok {
			var replicas int32
			...
			case int:
				replicas = int32(replicasTyped)
			default:
				return nil, errors.Errorf("failed to parse %s[%d].replicas from object", "."+strings.Join(v.path, "."), i)
			}
...
func (v *StatusVersions) Set(obj *unstructured.Unstructured, value []clusterv1.StatusVersion) error {
	if err := unstructured.SetNestedSlice(obj.UnstructuredContent(), toStatusVersionInterfaces(value), v.path...); err != nil {
		return errors.Wrapf(err, "failed to set path %s of object %v", "."+strings.Join(v.path, "."), obj.GroupVersionKind())
	}
	return nil
}

func toStatusVersionInterfaces(value []clusterv1.StatusVersion) []interface{} {
	interfaces := make([]interface{}, 0, len(value))
	for _, statusVersion := range value {
		item := map[string]interface{}{
			"version": statusVersion.Version,
		}
		if statusVersion.Replicas != nil {
			item["replicas"] = int64(*statusVersion.Replicas)
		}
...
// - if spec.version is greater than status.versions (status.version is used as fallback).
...
func (c *ControlPlaneContract) currentStatusVersion(obj *unstructured.Unstructured) (*string, error) {
	...
	if err == nil && len(statusVersions) > 0 {
		lowest := statusVersions[0].Version
		for i := 1; i < len(statusVersions); i++ {
			v := statusVersions[i].Version
			lowestSemver, lowestErr := semver.ParseTolerant(lowest)
