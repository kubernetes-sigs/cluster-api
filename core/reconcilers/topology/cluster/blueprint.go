...
func (r *Reconciler) getBlueprint(ctx context.Context, cluster *clusterv1.Cluster, clusterClass *clusterv1.ClusterClass) (_ *scope.ClusterBlueprint, reterr error) {
	...
	blueprint.InfrastructureClusterTemplate, err = r.getTemplate(ctx, blueprint.ClusterClass.Spec.Infrastructure.TemplateRef, clusterClass.Namespace, blueprint.ClusterClass.Spec.Infrastructure.Template)
	if err != nil {
	...
	blueprint.ControlPlane.Template, err = r.getTemplate(ctx, blueprint.ClusterClass.Spec.ControlPlane.TemplateRef, clusterClass.Namespace, blueprint.ClusterClass.Spec.ControlPlane.Template)
	if err != nil {
	...
	if blueprint.HasControlPlaneInfrastructureMachine() {
		blueprint.ControlPlane.InfrastructureMachineTemplate, err = r.getTemplate(ctx, blueprint.ClusterClass.Spec.ControlPlane.MachineInfrastructure.TemplateRef, clusterClass.Namespace, blueprint.ClusterClass.Spec.ControlPlane.MachineInfrastructure.Template)
		if err != nil {
	...
	for _, machineDeploymentClass := range blueprint.ClusterClass.Spec.Workers.MachineDeployments {
		...
		machineDeploymentBlueprint.InfrastructureMachineTemplate, err = r.getTemplate(ctx, machineDeploymentClass.Infrastructure.TemplateRef, clusterClass.Namespace, machineDeploymentClass.Infrastructure.Template)
		if err != nil {
		...
		machineDeploymentBlueprint.BootstrapTemplate, err = r.getTemplate(ctx, machineDeploymentClass.Bootstrap.TemplateRef, clusterClass.Namespace, machineDeploymentClass.Bootstrap.Template)
		if err != nil {
	...
	for _, machinePoolClass := range blueprint.ClusterClass.Spec.Workers.MachinePools {
		...
		machinePoolBlueprint.InfrastructureMachinePoolTemplate, err = r.getTemplate(ctx, machinePoolClass.Infrastructure.TemplateRef, clusterClass.Namespace, machinePoolClass.Infrastructure.Template)
		if err != nil {
		...
		// Get the bootstrap config template.
		machinePoolBlueprint.BootstrapTemplate, err = r.getTemplate(ctx, machinePoolClass.Bootstrap.TemplateRef, clusterClass.Namespace, machinePoolClass.Bootstrap.Template)
		if err != nil {
