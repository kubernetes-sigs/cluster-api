...
import (
	...
	names "sigs.k8s.io/cluster-api/internal/topology/names"
...
func (webhook *MachineDeployment) validate(oldMD, newMD *clusterv1.MachineDeployment) error {
	...
	if newMD.Spec.MachineNamingStrategy != nil {
		allErrs = append(allErrs, validateNamingStrategy(newMD.Spec.MachineNamingStrategy, specPath.Child("machineNamingStrategy"))...)
	}
...
func validateNamingStrategy(machineNamingStrategy *clusterv1.MachineNamingStrategy, pathPrefix *field.Path) field.ErrorList {
	var allErrs field.ErrorList
	...
	if machineNamingStrategy.Template != "" {
		...
		name, err := names.MachineDeploymentMachineNameGenerator(machineNamingStrategy.Template, "cluster", "machinedeployment").GenerateName()
		if err != nil {
			allErrs = append(allErrs,
				field.Invalid(
					pathPrefix.Child("template"),
