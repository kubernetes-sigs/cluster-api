...
func TestMachineSetReconciler_runPreflightChecks(t *testing.T) {
	...
	t.Run("should run preflight checks if the feature gate is enabled", func(t *testing.T) {
		tests := []struct {
			...
			{
				// A version that is set is always honored, the feature gate only controls if it can be
				// set, so the skip applies even with the feature gate off.
				name: "control plane version preflight check: should skip for a pinned machine deployment even if the feature gate is disabled",
				cluster: &clusterv1.Cluster{
					ObjectMeta: metav1.ObjectMeta{
						Namespace: ns,
					},
