	...
	if machineNamingStrategy.Template != "" {
		if !strings.Contains(machineNamingStrategy.Template, ".random") {
			allErrs = append(allErrs,
				field.Invalid(
					...
					"invalid ControlPlane name template, {{ .random }} variable is missed",
				))
			return allErrs
		}
		...
		if err != nil {
			...
			for _, err := range validation.IsDNS1123Subdomain(name) {
				allErrs = append(allErrs, field.Invalid(pathPrefix.Child("template"), machineNamingStrategy.Template, err))
			}
		}
	}
