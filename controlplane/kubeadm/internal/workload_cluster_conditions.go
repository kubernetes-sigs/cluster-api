...
func (w *Workload) updateExternalEtcdConditions(_ context.Context, controlPlane *ControlPlane) {
	...
	// Note: KCP is going to stop setting the `EtcdClusterAvailable` condition to true in case of external etcd.
	// This will allow tools managing the external etcd instance to use the `EtcdClusterAvailable` to report back status into
	...
	for _, node := range controlPlaneNodes.Items {
		...
		if !machine.ObjectMeta.DeletionTimestamp.IsZero() {
			...
			v1beta2conditions.Set(machine, metav1.Condition{
				...
				Reason: controlplanev1.KubeadmControlPlaneMachineEtcdMemberDeletingV1Beta2V1Beta2Reason,
			})
			continue
		}
		...
		if member == nil {
			...
			v1beta2conditions.Set(machine, metav1.Condition{
				...
				Status:  metav1.ConditionFalse,
				Reason:  controlplanev1.KubeadmControlPlaneMachineEtcdMemberNotHealthyV1Beta2Reason,
				Message: fmt.Sprintf("Etcd reports the cluster is composed by %s, but the etcd member hosted on this machine (%s) is not included", etcdutil.MemberNames(currentMembers), node.Name),
			})
			continue
		}
...
func (w *Workload) getCurrentEtcdMembers(ctx context.Context, machine *clusterv1.Machine, nodeName string) ([]*etcd.Member, error) {
	...
	if err != nil {
		...
		v1beta2conditions.Set(machine, metav1.Condition{
			...
			Status:  metav1.ConditionUnknown,
			Reason:  controlplanev1.KubeadmControlPlaneMachineEtcdMemberInspectionFailedV1Beta2Reason,
			Message: fmt.Sprintf("Failed get answer from the etcd member on the %s node: %s", nodeName, err.Error()),
		})
		...
		for i := range controlPlane.Machines {
			...
			for _, condition := range allMachinePodV1beta2Conditions {
				v1beta2conditions.Set(machine, metav1.Condition{
					...
					Status:  metav1.ConditionUnknown,
					Reason:  controlplanev1.KubeadmControlPlaneMachinePodInspectionFailedV1Beta2Reason,
					Message: fmt.Sprintf("Failed to get the Node hosting the %s", strings.TrimSuffix(condition, "Healthy")),
				})
			}
		}
		...
		if !machine.ObjectMeta.DeletionTimestamp.IsZero() {
			...
			for _, condition := range allMachinePodV1beta2Conditions {
				v1beta2conditions.Set(machine, metav1.Condition{
					Type:   condition,
					Status: metav1.ConditionFalse,
					Reason: controlplanev1.KubeadmControlPlaneMachinePodDeletingV1Beta2V1Beta2Reason,
				})
			}
			continue
		}
