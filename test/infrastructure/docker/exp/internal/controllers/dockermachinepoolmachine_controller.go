...
// DockerMachinePoolMachineReconciler reconciles a DockerMachinePoolMachine object.
...
// Reconcile handles DockerMachinePoolMachine events.
func (r *DockerMachinePoolMachineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, rerr error) {
	log := ctrl.LoggerFrom(ctx)
	ctx = container.RuntimeInto(ctx, r.ContainerRuntime)

	log.Info("Reconciling DockerMachinePoolMachine")

	// Fetch the DockerMachinePoolMachine instance.
	dockerMachinePoolMachine := &infraexpv1.DockerMachinePoolMachine{}
	if err := r.Client.Get(ctx, req.NamespacedName, dockerMachinePoolMachine); err != nil {
		if apierrors.IsNotFound(err) {
			log.Info("DMPM not found in reconcile, returning", "DMPM", req.NamespacedName)
		}
		return ctrl.Result{}, err
	}

	// Fetch the DMPM.
	machine, err := util.GetOwnerMachine(ctx, r.Client, dockerMachinePoolMachine.ObjectMeta)
	if err != nil {
		return ctrl.Result{}, err
	}
	if machine == nil {
		log.Info("Waiting for Machine Controller to set OwnerRef on DockerMachinePoolMachine")
	...
	if err != nil {
		log.Info("DockerMachinePoolMachine owner Machine is missing cluster label or cluster does not exist")
		return ctrl.Result{}, err
	}
	if cluster == nil {
	...
	if annotations.IsPaused(cluster, dockerMachinePoolMachine) {
	...
	dockerClusterName := client.ObjectKey{
		Namespace: dockerMachinePoolMachine.Namespace,
	...
	patchHelper, err := patch.NewHelper(dockerMachinePoolMachine, r.Client)
	if err != nil {
		return ctrl.Result{}, err
	}
	// Always attempt to Patch the DockerMachinePoolMachine object and status after each reconciliation.
	defer func() {
		if err := patchDockerMachine(ctx, patchHelper, dockerMachinePoolMachine); err != nil {
	...
	if !controllerutil.ContainsFinalizer(dockerMachinePoolMachine, infrav1.MachineFinalizer) {
		controllerutil.AddFinalizer(dockerMachinePoolMachine, infrav1.MachineFinalizer)
		log.Info("Adding finalizer to the DockerMachinePoolMachine, returning", "dmpm", dockerMachinePoolMachine.Name)
		return ctrl.Result{}, nil
	}
	...
	if !cluster.Status.InfrastructureReady {
		...
		conditions.MarkFalse(dockerMachinePoolMachine, infrav1.ContainerProvisionedCondition, infrav1.WaitingForClusterInfrastructureReason, clusterv1.ConditionSeverityInfo, "")
		return ctrl.Result{}, nil
	}

	// Create a helper for managing the docker container hosting the machine.
	externalMachine, err := docker.NewMachine(ctx, cluster, machine.Name, dockerMachinePoolMachine.Spec.CustomImage, nil)
	if err != nil {
	...
	log.Info(fmt.Sprintf("Deletion timestamp for machine %+v w/ timestamp %+v", machine.Name, dockerMachinePoolMachine.DeletionTimestamp))
	log.Info("Is zero? ", "bool", dockerMachinePoolMachine.ObjectMeta.DeletionTimestamp.IsZero())
	if !dockerMachinePoolMachine.ObjectMeta.DeletionTimestamp.IsZero() {
		return r.reconcileDelete(ctx, machine, dockerMachinePoolMachine, externalMachine, externalLoadBalancer)
	}

	// Handle non-deleted machines
	log.Info("Reconciling dmpm normally")
	return r.reconcileNormal(ctx, cluster, machine, dockerMachinePoolMachine, externalMachine, externalLoadBalancer)
}

func patchDockerMachine(ctx context.Context, patchHelper *patch.Helper, dockerMachine *infraexpv1.DockerMachinePoolMachine) error {
...
func (r *DockerMachinePoolMachineReconciler) reconcileNormal(ctx context.Context, cluster *clusterv1.Cluster, machine *clusterv1.Machine, dockerMachine *infraexpv1.DockerMachinePoolMachine, externalMachine *docker.Machine, externalLoadBalancer *docker.LoadBalancer) (res ctrl.Result, retErr error) {
	log := ctrl.LoggerFrom(ctx)
...
func (r *DockerMachinePoolMachineReconciler) reconcileDelete(ctx context.Context, machine *clusterv1.Machine, dockerMachine *infraexpv1.DockerMachinePoolMachine, externalMachine *docker.Machine, externalLoadBalancer *docker.LoadBalancer) (ctrl.Result, error) {
	log := ctrl.LoggerFrom(ctx)

	log.Info("reconciling delete for", "machine", machine.Name, "dockerMachine", dockerMachine.Name)
	...
	if err := patchDockerMachine(ctx, patchHelper, dockerMachine); err != nil {
		return ctrl.Result{}, errors.Wrap(err, "failed to patch DockerMachinePoolMachine")
	}

	// delete the machine
	log.Info("deleting dmpm", "machine", machine.Name, "dockerMachine", dockerMachine.Name)
	if err := externalMachine.Delete(ctx); err != nil {
		return ctrl.Result{}, errors.Wrap(err, "failed to delete DockerMachinePoolMachine")
	}
...
func (r *DockerMachinePoolMachineReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, options controller.Options) error {
	clusterToDockerMachinePoolMachines, err := util.ClusterToObjectsMapper(mgr.GetClient(), &infraexpv1.DockerMachinePoolMachineList{}, mgr.GetScheme())
	if err != nil {
		return err
	}

	c, err := ctrl.NewControllerManagedBy(mgr).
		For(&infraexpv1.DockerMachinePoolMachine{}).
		WithOptions(options).
		WithEventFilter(predicates.ResourceNotPaused(ctrl.LoggerFrom(ctx))).
		Watches(
			&source.Kind{Type: &clusterv1.Machine{}},
			handler.EnqueueRequestsFromMapFunc(util.MachineToInfrastructureMapFunc(infrav1.GroupVersion.WithKind("DockerMachinePoolMachine"))),
		).
		Watches(
			&source.Kind{Type: &infrav1.DockerCluster{}},
			handler.EnqueueRequestsFromMapFunc(r.DockerClusterToDockerMachinePoolMachines),
		).
		Build(r)
	if err != nil {
		return err
	...
	return c.Watch(
		...
		handler.EnqueueRequestsFromMapFunc(clusterToDockerMachinePoolMachines),
		predicates.ClusterUnpausedAndInfrastructureReady(ctrl.LoggerFrom(ctx)),
	)
}

// DockerClusterToDockerMachinePoolMachines is a handler.ToRequestsFunc to be used to enqeue
// requests for reconciliation of DockerMachines.
func (r *DockerMachinePoolMachineReconciler) DockerClusterToDockerMachinePoolMachines(o client.Object) []ctrl.Request {
	result := []ctrl.Request{}
	...
	// machinePoolMachineList := &infraexpv1.MachinePoolList{}
...
func (r *DockerMachinePoolMachineReconciler) getBootstrapData(ctx context.Context, machine *clusterv1.Machine) (string, bootstrapv1.Format, error) {
	if machine.Spec.Bootstrap.DataSecretName == nil {
...
func setMachineAddress(ctx context.Context, dockerMachinePoolMachine *infraexpv1.DockerMachinePoolMachine, externalMachine *docker.Machine) error {
	machineAddress, err := externalMachine.Address(ctx)
	if err != nil {
		return err
	}

	dockerMachinePoolMachine.Status.Addresses = []clusterv1.MachineAddress{
		{
