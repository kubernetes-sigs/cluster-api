...
func (r *Reconciler) getCurrentMachineDeploymentState(ctx context.Context, blueprintMachineDeployments map[string]*scope.MachineDeploymentBlueprint, cluster *clusterv1.Cluster) (map[string]*scope.MachineDeploymentState, error) {
	...
	for i := range md.Items {
		...
		if !ok || mdTopologyName == "" {
			return nil, fmt.Errorf("failed to find label %s in %s", clusterv1.ClusterTopologyMachineDeploymentNameLabel, klog.KObj(m))
		}
		...
		if _, ok := state[mdTopologyName]; ok {
			return nil, fmt.Errorf("duplicate %s found for label %s: %s", klog.KObj(m), clusterv1.ClusterTopologyMachineDeploymentNameLabel, mdTopologyName)
		}

		// Gets the bootstrapRef.
		bootstrapRef := m.Spec.Template.Spec.Bootstrap.ConfigRef
		if bootstrapRef == nil {
			return nil, fmt.Errorf("%s does not have a reference to a Bootstrap Config", klog.KObj(m))
		}
		// Gets the infraRef.
		infraRef := &m.Spec.Template.Spec.InfrastructureRef
		if infraRef.Name == "" {
			return nil, fmt.Errorf("%s does not have a reference to a InfrastructureMachineTemplate", klog.KObj(m))
		}
		...
		if mdTopologyExistsInCluster, mdClassName := getMDClassName(cluster, mdTopologyName); mdTopologyExistsInCluster {
			...
			if err != nil {
				return nil, errors.Wrap(err, fmt.Sprintf("%s Bootstrap reference could not be retrieved", klog.KObj(m)))
			}
			infraRef, err = alignRefAPIVersion(mdBluePrint.InfrastructureMachineTemplate, infraRef)
			if err != nil {
				return nil, errors.Wrap(err, fmt.Sprintf("%s Infrastructure reference could not be retrieved", klog.KObj(m)))
			}
		}
		...
		if !labels.IsTopologyOwned(bootstrapTemplate) {
			return nil, fmt.Errorf("BootstrapTemplate object %s referenced from MD %s is not topology owned", klog.KObj(bootstrapTemplate), klog.KObj(m))
		}
		...
		if !labels.IsTopologyOwned(infraMachineTemplate) {
			return nil, fmt.Errorf("InfrastructureMachineTemplate object %s referenced from MD %s is not topology owned", klog.KObj(infraMachineTemplate), klog.KObj(m))
		}
		...
		if err := r.Client.Get(ctx, client.ObjectKey{Namespace: m.Namespace, Name: m.Name}, mhc); err != nil {
			...
			if !apierrors.IsNotFound(err) {
				return nil, errors.Wrap(err, fmt.Sprintf("failed to get MachineHealthCheck for %s", klog.KObj(m)))
			}
		}
...
func (r *Reconciler) getCurrentMachinePoolState(ctx context.Context, blueprintMachinePools map[string]*scope.MachinePoolBlueprint, cluster *clusterv1.Cluster) (map[string]*scope.MachinePoolState, error) {
	...
	for i := range mp.Items {
		...
		if !ok || mpTopologyName == "" {
			return nil, fmt.Errorf("failed to find label %s in %s", clusterv1.ClusterTopologyMachinePoolNameLabel, klog.KObj(m))
		}
		...
		if _, ok := state[mpTopologyName]; ok {
			return nil, fmt.Errorf("duplicate %s found for label %s: %s", klog.KObj(m), clusterv1.ClusterTopologyMachinePoolNameLabel, mpTopologyName)
		}
		...
		infraRef := &m.Spec.Template.Spec.InfrastructureRef
		if infraRef.Name == "" {
			return nil, fmt.Errorf("%s does not have a reference to a InfrastructureMachinePool", klog.KObj(m))
		}
		...
		if !labels.IsTopologyOwned(bootstrapObject) {
			return nil, fmt.Errorf("bootstrap object %s referenced from MP %s is not topology owned", klog.KObj(bootstrapObject), klog.KObj(m))
		}
		...
		if !labels.IsTopologyOwned(infraMachinePoolObject) {
			return nil, fmt.Errorf("InfrastructureMachinePool object %s referenced from MP %s is not topology owned", klog.KObj(infraMachinePoolObject), klog.KObj(m))
		}
