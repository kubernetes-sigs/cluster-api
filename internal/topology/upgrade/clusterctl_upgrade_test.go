...
func setupT2CRDAndWebHooks(g *WithT, ns *corev1.Namespace, t1CheckObj *testt1v1beta1.TestResource, t1CheckTemplate *testt1v1beta1.TestResourceTemplate) (client.Client, *admissionv1.MutatingWebhookConfiguration, *admissionv1.MutatingWebhookConfiguration) {
	...
	err = addOrReplaceConversionWebhookHandler(ctx, env.GetClient(), "testresourcetemplates.test.cluster.x-k8s.io", conversion.NewWebhookHandler(t2Scheme, conversion.NewRegistry()))
	g.Expect(err).ToNot(HaveOccurred())

	err = addOrReplaceConversionWebhookHandler(ctx, env.GetClient(), "testresources.test.cluster.x-k8s.io", conversion.NewWebhookHandler(t2Scheme, conversion.NewRegistry()))
	g.Expect(err).ToNot(HaveOccurred())
