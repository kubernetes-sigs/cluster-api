...
func (r *Reconciler) getBlueprint(ctx context.Context, cluster *clusterv1.Cluster, clusterClass *clusterv1.ClusterClass) (_ *scope.ClusterBlueprint, reterr error) {
	...
	for _, machineDeploymentClass := range blueprint.ClusterClass.Spec.Workers.MachineDeployments {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "failed to get bootstrap config template for %s, MachineDeployment class %q", klog.KObj(blueprint.ClusterClass), machineDeploymentClass.Class)
		}
	...
	for _, machinePoolClass := range blueprint.ClusterClass.Spec.Workers.MachinePools {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "failed to get InfrastructureMachinePoolTemplate for %s, MachinePool class %q", klog.KObj(blueprint.ClusterClass), machinePoolClass.Class)
		}

		// Get the bootstrap config template.
		machinePoolBlueprint.BootstrapTemplate, err = r.getReference(ctx, machinePoolClass.Template.Bootstrap.Ref)
		if err != nil {
			return nil, errors.Wrapf(err, "failed to get bootstrap config for %s, MachinePool class %q", klog.KObj(blueprint.ClusterClass), machinePoolClass.Class)
		}
