...
# This script pins a go.mod dependency to a specific SHA or tag via a replace directive.
...
# Usage:
#   hack/scripts/release/pin-dependency.sh $MODULE $SHA-OR-TAG
#
# Example:
#   hack/scripts/release/pin-dependency.sh github.com/docker/docker 501cb131a7b7
...
if [[ -z "${dep}" || -z "${sha}" ]]; then
  echo "Usage:"
  echo "  hack/scripts/release/pin-dependency.sh \$MODULE \$SHA-OR-TAG"
  echo ""
  echo "Example:"
  echo "  hack/scripts/release/pin-dependency.sh github.com/docker/docker 501cb131a7b7"
  echo ""
  exit 1
fi
