...
func TestDevMachineTemplateInvalid(t *testing.T) {
	oldTemplate := infrav1.DevMachineTemplate{
	...
	newTemplate.Spec.Template.Spec.Backend.Docker.ExtraMounts = append(newTemplate.Spec.Template.Spec.Backend.Docker.ExtraMounts, []infrav1.Mount{{ContainerPath: "/var/run/docker.sock", HostPath: "/var/run/docker.sock"}}...)
	...
	tests := []struct {
		name        string
		newTemplate *infrav1.DevMachineTemplate
		oldTemplate *infrav1.DevMachineTemplate
