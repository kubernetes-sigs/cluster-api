...
// Note: The ClusterClass is optional; if it is nil, the version ceiling is always validated.
func (webhook *Cluster) validateTopologyVersionUpdate(ctx context.Context, oldCluster, newCluster *clusterv1.Cluster, clusterClass *clusterv1.ClusterClass, fldPath *field.Path) (admission.Warnings, field.ErrorList) {
	...
	// Note: parsed strictly, like in validateTopologyVersion.
	inVersion, err := semver.Parse(strings.TrimPrefix(newCluster.Spec.Topology.Version, "v"))
	if err != nil {
		allErrs = append(
			allErrs,
			field.Invalid(
	...
	oldVersion, err := semver.Parse(strings.TrimPrefix(oldCluster.Spec.Topology.Version, "v"))
	if err != nil {
	...
	// * the ClusterClass is not available or
	...
	shouldValidateVersionCeiling := clusterClass == nil ||
		(len(clusterClass.Spec.KubernetesVersions) == 0 && clusterClass.Spec.Upgrade.External.GenerateUpgradePlanExtension == "")
