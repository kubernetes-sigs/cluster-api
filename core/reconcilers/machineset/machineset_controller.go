...
func (r *Reconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, options controller.Options) error {
	...
	c, err := capicontrollerutil.NewControllerManagedBy(mgr, predicateLog).
		WithDynamicCache(r.DynamicCache).
...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (retres ctrl.Result, reterr error) {
	...
	defer func() {
		if err := r.updateStatus(ctx, s); err != nil {
			reterr = kerrors.NewAggregate([]error{reterr, pkgerrors.Wrapf(err, "failed to update status")})
		}

		if err := r.reconcileV1Beta1Status(ctx, s); err != nil {
			reterr = kerrors.NewAggregate([]error{reterr, pkgerrors.Wrapf(err, "failed to update deprecated v1beta1 status")})
		}
...
func wrapErrMachineSetReconcileBlockingFunc(f machineSetReconcileBlockingFunc, msg string) machineSetReconcileBlockingFunc {
	return func(ctx context.Context, s *scope) (ctrl.Result, bool, error) {
		...
		return res, stopReconcile, pkgerrors.Wrap(err, msg)
	}
}
...
func wrapErrMachineSetReconcileFunc(f machineSetReconcileFunc, msg string) machineSetReconcileBlockingFunc {
	return func(ctx context.Context, s *scope) (ctrl.Result, bool, error) {
		res, err := f(ctx, s)
		return res, false, pkgerrors.Wrap(err, msg)
	}
}
...
func (r *Reconciler) triggerInPlaceUpdate(ctx context.Context, s *scope) (ctrl.Result, error) {
	...
	for _, machine := range s.machines {
		...
		if err := r.Client.Patch(ctx, machine, client.MergeFrom(orig)); err != nil {
			errs = append(errs, pkgerrors.Wrapf(err, "failed to complete triggering in-place update for Machine %s", klog.KObj(machine)))
			continue
		}
...
func (r *Reconciler) completeMoveMachine(ctx context.Context, s *scope, currentMachine *clusterv1.Machine) error {
	...
	if err != nil {
		return pkgerrors.Wrap(err, "could not compute desired Machine")
	}
	...
	_, currentInfraMachine, err := r.DynamicCache.GetContractObject(ctx, currentMachine.Spec.InfrastructureRef, currentMachine.Namespace, setup.DynamicCacheInfraMachineCacheOptionsID)
	if err != nil {
	...
	if err != nil {
		return pkgerrors.Wrap(err, "could not compute desired InfraMachine")
	}
	...
	if currentMachine.Spec.Bootstrap.ConfigRef.IsDefined() {
		...
		_, currentBootstrapConfig, err := r.DynamicCache.GetContractObject(ctx, currentMachine.Spec.Bootstrap.ConfigRef, currentMachine.Namespace, setup.DynamicCacheBootstrapConfigCacheOptionsID)
		if err != nil {
		...
		if err != nil {
			return pkgerrors.Wrap(err, "could not compute desired BootstrapConfig")
		}
	...
	if err := ssa.Patch(ctx, r.Client, machineSetManagerName, desiredInfraMachine); err != nil {
		return pkgerrors.Wrapf(err, "failed to complete triggering in-place update for Machine %s", klog.KObj(desiredMachine))
	}
	...
	for _, machine := range machineList {
		if machine.DeletionTimestamp.IsZero() {
			if err := r.Client.Delete(ctx, machine); err != nil && !apierrors.IsNotFound(err) {
				return ctrl.Result{}, pkgerrors.Wrapf(err, "failed to delete Machine %s", klog.KObj(machine))
			}
...
func (r *Reconciler) getAndAdoptMachinesForMachineSet(ctx context.Context, s *scope) (ctrl.Result, bool, error) {
	...
	if err != nil {
		return ctrl.Result{}, true, pkgerrors.Wrapf(err, "failed to convert MachineSet %q label selector to a map", machineSet.Name)
	}
	...
	if err != nil {
		return ctrl.Result{}, true, pkgerrors.Wrap(err, "failed to list machines")
	}
...
func (r *Reconciler) syncMachines(ctx context.Context, s *scope) (ctrl.Result, bool, error) {
	...
	for i := range machines {
		...
		if !m.DeletionTimestamp.IsZero() {
			...
			coreadmission.DefaultMachineNodeDeletionTimeoutSeconds(m) // Default to avoid unnecessary patch calls if field is not set on MS.
		...
		if !anyManagedFieldIssueMitigated {
			...
			if err != nil {
				return ctrl.Result{}, true, pkgerrors.Wrap(err, "failed to update Machine: failed to compute desired Machine")
			}
			...
			if err != nil {
				...
				return ctrl.Result{}, true, pkgerrors.Wrapf(err, "failed to update Machine %q", klog.KObj(updatedMachine))
			}
			machines[i] = updatedMachine
		}

		infraMachineGVK, infraMachine, err := r.DynamicCache.GetContractObject(ctx, m.Spec.InfrastructureRef, m.Namespace, setup.DynamicCacheInfraMachineCacheOptionsID)
		if err != nil {
		...
		infraMachineClient, err := r.DynamicCache.GetWriter(ctx, infraMachineGVK, infraMachine)
		if err != nil {
			return ctrl.Result{}, true, pkgerrors.Wrapf(err, "failed to create client for InfrastructureMachine %s %s",
				m.Spec.InfrastructureRef.Kind, klog.KRef(m.Namespace, m.Spec.InfrastructureRef.Name))
		}
		managedFieldIssueMitigated, err = ssa.MitigateManagedFieldsIssue(ctx, infraMachineClient, infraMachine, machineSetMetadataManagerName)
		if err != nil {
			return ctrl.Result{}, true, err
		}
		anyManagedFieldIssueMitigated = anyManagedFieldIssueMitigated || managedFieldIssueMitigated
		if !anyManagedFieldIssueMitigated {
			...
			if err := ssa.MigrateManagedFields(ctx, infraMachineClient, infraMachine, machineSetManagerName, machineSetMetadataManagerName); err != nil {
				return ctrl.Result{}, true, pkgerrors.Wrapf(err, "failed to clean up managedFields of InfrastructureMachine %s", klog.KObj(infraMachine))
			}
			// Update in-place mutating fields on InfrastructureMachine.
			if err := r.updateLabelsAndAnnotations(ctx, infraMachineClient, infraMachineGVK, infraMachine, machineSet); err != nil {
				return ctrl.Result{}, true, pkgerrors.Wrapf(err, "failed to update InfrastructureMachine %s", klog.KObj(infraMachine))
			}
		}

		if m.Spec.Bootstrap.ConfigRef.IsDefined() {
			bootstrapConfigGVK, bootstrapConfig, err := r.DynamicCache.GetContractObject(ctx, m.Spec.Bootstrap.ConfigRef, m.Namespace, setup.DynamicCacheBootstrapConfigCacheOptionsID)
			if err != nil {
			...
			bootstrapConfigClient, err := r.DynamicCache.GetWriter(ctx, bootstrapConfigGVK, bootstrapConfig)
			if err != nil {
				return ctrl.Result{}, true, pkgerrors.Wrapf(err, "failed to create client for InfrastructureMachine %s %s",
					m.Spec.InfrastructureRef.Kind, klog.KRef(m.Namespace, m.Spec.InfrastructureRef.Name))
			}
			managedFieldIssueMitigated, err = ssa.MitigateManagedFieldsIssue(ctx, bootstrapConfigClient, bootstrapConfig, machineSetMetadataManagerName)
			if err != nil {
				return ctrl.Result{}, true, err
			}
			anyManagedFieldIssueMitigated = anyManagedFieldIssueMitigated || managedFieldIssueMitigated
			if !anyManagedFieldIssueMitigated {
				...
				//       Given that this was introduced in CAPI v1.12 and our n-3 upgrade policy this can
				//       be removed with CAPI v1.15.
				if err := ssa.MigrateManagedFields(ctx, bootstrapConfigClient, bootstrapConfig, machineSetManagerName, machineSetMetadataManagerName); err != nil {
					return ctrl.Result{}, true, pkgerrors.Wrapf(err, "failed to clean up managedFields of BootstrapConfig %s", klog.KObj(bootstrapConfig))
				}
				// Update in-place mutating fields on BootstrapConfig.
				if err := r.updateLabelsAndAnnotations(ctx, bootstrapConfigClient, bootstrapConfigGVK, bootstrapConfig, machineSet); err != nil {
					return ctrl.Result{}, true, pkgerrors.Wrapf(err, "failed to update BootstrapConfig %s", klog.KObj(bootstrapConfig))
				}
			}
		}
	}
...
func (r *Reconciler) cleanupOrphanedBootstrapConfigsInfraMachines(ctx context.Context, s *scope) error {
	...
	infraMachineGVK, infraMachines, err := r.listMSOwnedObjectsFromContractVersionedRef(ctx, s.machineSet.Spec.Template.Spec.InfrastructureRef, setup.DynamicCacheInfraMachineCacheOptionsID, s.machineSet)
	if err != nil {
		return pkgerrors.Wrapf(err, "failed to cleanup orphaned objects")
	}

	// Get BootstrapConfigs of the MachineSet.
	bootstrapConfigs := map[string]client.Object{}
	var bootstrapConfigGVK schema.GroupVersionKind
	if s.machineSet.Spec.Template.Spec.Bootstrap.ConfigRef.IsDefined() {
		bootstrapConfigGVK, bootstrapConfigs, err = r.listMSOwnedObjectsFromContractVersionedRef(ctx, s.machineSet.Spec.Template.Spec.Bootstrap.ConfigRef, setup.DynamicCacheBootstrapConfigCacheOptionsID, s.machineSet)
		if err != nil {
			return pkgerrors.Wrapf(err, "failed to cleanup orphaned objects")
		}
	}
	...
	for _, infraMachine := range infraMachines {
		...
		if err := r.Client.Delete(ctx, external.ToUnstructured(infraMachineGVK, infraMachine)); err != nil && !apierrors.IsNotFound(err) {
			errs = append(errs, pkgerrors.Wrapf(err, "failed to delete %s %s", infraMachineGVK, klog.KObj(infraMachine)))
		}
	}
	for _, bootstrapConfig := range bootstrapConfigs {
		...
		if err := r.Client.Delete(ctx, external.ToUnstructured(bootstrapConfigGVK, bootstrapConfig)); err != nil && !apierrors.IsNotFound(err) {
			errs = append(errs, pkgerrors.Wrapf(err, "failed to delete %s %s", bootstrapConfigGVK, klog.KObj(bootstrapConfig)))
		}
	}

	if len(errs) > 0 {
		return pkgerrors.Wrapf(kerrors.NewAggregate(errs), "failed to cleanup orphaned objects")
	}

	return nil
}

func (r *Reconciler) listMSOwnedObjectsFromContractVersionedRef(ctx context.Context, ref clusterv1.ContractVersionedObjectReference, objectType external.ObjectType, ms *clusterv1.MachineSet) (schema.GroupVersionKind, map[string]client.Object, error) {
	if !ref.IsDefined() {
		return schema.GroupVersionKind{}, nil, pkgerrors.Errorf("failed to list objects from ref: object reference not set")
	}

	selectorMap, err := metav1.LabelSelectorAsMap(&ms.Spec.Selector)
	if err != nil {
		return schema.GroupVersionKind{}, nil, pkgerrors.Wrapf(err, "failed to convert MachineSet %q label selector to a map", ms.Name)
	}
	...
	objGVK, objList, err := r.DynamicCache.ListContractObject(ctx, gk, objectType, client.InNamespace(ms.Namespace), client.MatchingLabels(selectorMap))
	if err != nil {
	...
	err = meta.EachListItem(objList, func(obj runtime.Object) error {
		...
		if util.HasOwnerRef(clientObj.GetOwnerReferences(), metav1.OwnerReference{
			APIVersion: clusterv1.GroupVersion.String(),
			Kind:       "MachineSet",
			Name:       ms.Name,
			UID:        ms.UID,
		}) {
			objs[clientObj.GetName()] = clientObj
		}
		return nil
	})
	if err != nil {
...
func (r *Reconciler) createMachines(ctx context.Context, s *scope, machinesToAdd int) (ctrl.Result, error) {
	...
	for i := range machinesToAdd {
		...
		if computeMachineErr != nil {
			...
			return ctrl.Result{}, pkgerrors.Wrap(computeMachineErr, "failed to create Machine: failed to compute desired Machine")
		}
		...
		if ms.Spec.Template.Spec.Bootstrap.ConfigRef.IsDefined() {
			...
			if err != nil {
				...
				return ctrl.Result{}, pkgerrors.Wrapf(err, "failed to clone bootstrap configuration from %s %s while creating a Machine",
			...
			r.controller.DeferNextReconcileUntilCacheUpToDate(s.machineSet, capicontrollerutil.DynamicCacheStructuredObject(bootstrapConfig.GroupVersionKind()), bootstrapConfig.GetResourceVersion())
		}
		...
		if err != nil {
			...
			if bootstrapRef.IsDefined() {
				...
				if err := r.Client.Delete(ctx, bootstrapConfig); err != nil && !apierrors.IsNotFound(err) {
					deleteErr = pkgerrors.Wrapf(err, "failed to cleanup %s %s after %s creation failed", bootstrapRef.Kind, klog.KRef(ms.Namespace, bootstrapRef.Name), ms.Spec.Template.Spec.InfrastructureRef.Kind)
				}
			}
			...
			return ctrl.Result{}, kerrors.NewAggregate([]error{pkgerrors.Wrapf(err, "failed to clone infrastructure machine from %s %s while creating a Machine",
		...
		r.controller.DeferNextReconcileUntilCacheUpToDate(s.machineSet, capicontrollerutil.DynamicCacheStructuredObject(infraMachine.GroupVersionKind()), infraMachine.GetResourceVersion())
		...
		if err := ssa.Patch(ctx, r.Client, machineSetManagerName, machine); err != nil {
			...
			if bootstrapRef.IsDefined() {
				if err := r.Client.Delete(ctx, bootstrapConfig); !apierrors.IsNotFound(err) {
					errs = append(errs, pkgerrors.Wrapf(err, "failed to cleanup %s %s after Machine creation failed", bootstrapRef.Kind, klog.KRef(ms.Namespace, bootstrapRef.Name)))
				}
			}
...
func (r *Reconciler) startMoveMachines(ctx context.Context, s *scope, targetMSName string, machinesToMove int) (ctrl.Result, error) {
	...
	if err := r.Client.Get(ctx, client.ObjectKey{Name: targetMSName, Namespace: ms.Namespace}, targetMS); err != nil {
		return ctrl.Result{}, pkgerrors.Wrapf(err, "failed to get MachineSet %s, which is the target for the move operation", targetMSName)
	}
	...
	if !sourcesSet.Has(ms.Name) {
		return ctrl.Result{}, pkgerrors.Errorf("MachineSet %s is set to move replicas to %s, but %[2]s only accepts Machines from %s", ms.Name, targetMS.Name, validSourceMSs)
	}
	...
	for _, machine := range machinesToMoveByPriority {
		...
		if collections.IsUnhealthyAndOwnerRemediated(machine) && machine.DeletionTimestamp.IsZero() {
			...
			deletedMachine, err := r.machineClientWithDeleteResponse.Delete(ctx, machine)
			if err != nil {
				errs = append(errs, pkgerrors.Wrapf(err, "failed to delete Machine %s (tried to delete the Machine instead of moving it to MachineSet %s because Machine is marked for remediation)", klog.KObj(machine), klog.KObj(targetMS)))
				continue
			}
			if deletedMachine != nil {
				r.controller.DeferNextReconcileUntilCacheUpToDate(s.machineSet, capicontrollerutil.StructuredObject(clusterv1.GroupVersion, "Machine"), deletedMachine.GetResourceVersion())
		...
		if !ok {
			return ctrl.Result{}, pkgerrors.Errorf("MachineSet %s does not have the %s label", targetMS.Name, clusterv1.MachineDeploymentUniqueLabel)
		}
		...
		if err := r.Client.Patch(ctx, machine, client.MergeFrom(orig)); err != nil {
			errs = append(errs, pkgerrors.Wrapf(err, "failed to trigger in-place update for Machine %s by moving to MachineSet %s", klog.KObj(machine), klog.KObj(targetMS)))
			continue
		}
		r.controller.DeferNextReconcileUntilCacheUpToDate(s.machineSet, capicontrollerutil.StructuredObject(clusterv1.GroupVersion, "Machine"), machine.ResourceVersion)
	}
...
func (r *Reconciler) computeDesiredMachine(machineSet *clusterv1.MachineSet, existingMachine *clusterv1.Machine) (*clusterv1.Machine, error) {
	...
	if machineSet.Spec.MachineNaming.Template != "" {
		...
		if !strings.Contains(nameTemplate, "{{ .random }}") {
			return nil, pkgerrors.New("cannot generate Machine name: {{ .random }} is missing in machineNaming.template")
		}
	}
	...
	if err != nil {
		return nil, pkgerrors.Wrap(err, "failed to generate Machine name")
	}
...
func (r *Reconciler) updateLabelsAndAnnotations(ctx context.Context, c ssa.WriterWithScheme, objGVK schema.GroupVersionKind, obj client.Object, machineSet *clusterv1.MachineSet) error {
...
func (r *Reconciler) getMachineSetsForMachine(ctx context.Context, m *clusterv1.Machine) ([]*clusterv1.MachineSet, error) {
	...
	if err := r.Client.List(ctx, msList, client.InNamespace(m.Namespace)); err != nil {
		return nil, pkgerrors.Wrapf(err, "failed to list MachineSets")
	}
...
func (r *Reconciler) reconcileV1Beta1Status(ctx context.Context, s *scope) error {
	...
	if ms.Spec.Replicas == nil {
		return pkgerrors.New("Cannot update status when MachineSet spec.replicas is not set")
	}
...
func (r *Reconciler) getMachineNode(ctx context.Context, cluster *clusterv1.Cluster, machine *clusterv1.Machine) (*corev1.Node, error) {
	...
	if err := remoteClient.Get(ctx, client.ObjectKey{Name: machine.Status.NodeRef.Name}, node); err != nil {
		return nil, pkgerrors.Wrapf(err, "error retrieving node %s for machine %s/%s", machine.Status.NodeRef.Name, machine.Namespace, machine.Name)
	}
	return node, nil
}
...
func (r *Reconciler) reconcileUnhealthyMachines(ctx context.Context, s *scope) (ctrl.Result, error) {
	...
	for _, m := range machinesToRemediate {
		if err := r.Client.Delete(ctx, m); err != nil && !apierrors.IsNotFound(err) {
			errs = append(errs, pkgerrors.Wrapf(err, "failed to delete Machine %s", klog.KObj(m)))
		}
		// Note: We intentionally log after Delete because we want this log line to show up only after DeletionTimestamp has been set.
		// Also, setting DeletionTimestamp doesn't mean the Machine is actually deleted (deletion takes some time).
	...
	if len(errs) > 0 {
		return ctrl.Result{}, pkgerrors.Wrapf(kerrors.NewAggregate(errs), "failed to delete unhealthy Machines")
	}
...
func patchMachineConditions(ctx context.Context, c client.Client, machines []*clusterv1.Machine, condition metav1.Condition, v1beta1condition *clusterv1.Condition) error {
	...
	if len(errs) > 0 {
		return pkgerrors.Wrapf(kerrors.NewAggregate(errs), "failed to patch Machines")
	}
...
func (r *Reconciler) computeDesiredBootstrapConfig(ctx context.Context, ms *clusterv1.MachineSet, machine *clusterv1.Machine, existingBootstrapConfig client.Object) (*unstructured.Unstructured, error) {
	...
	if err != nil {
		return nil, pkgerrors.Wrap(err, "failed to compute desired BootstrapConfig")
	}
